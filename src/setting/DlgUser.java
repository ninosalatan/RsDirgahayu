...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[K]Referensi Pendaftaran Mobile JKN","[K]Batal Pendaftaran Mobile JKN","[A]Transfusi Darah","[A]Gas Oksigen"
        };
        ...
        tabMode=new DefaultTableModel(null,row){
             ...
             public Class getColumnClass(int columnIndex) {
            	 //System.out.println("type:"+types.length);
        ...
        //System.out.println("type:"+row.length);
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "batal_pendaftaran_mobilejkn_bpjs='"+tbUser.getValueAt(i,718).toString()+"',labu_darah='"+tbUser.getValueAt(i,719).toString()+"',oksigen='"+tbUser.getValueAt(i,720).toString()+"'");
            }            
            tampil();
            emptTeks();
        }
                        ...
                        if (reply == JOptionPane.YES_OPTION) {
                            ...
                            if(i!= -1){
                                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                    ...
                                    +"labu_darah='"+tbUser.getValueAt(barisdicopy,719).toString()+"',"
                                    + "oksigen='"+tbUser.getValueAt(barisdicopy,720).toString()+"'");
                            }    
                            userdicopy="";
                            copyhakakses="";
                            barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement("select AES_DECRYPT(id_user,'nur'),AES_DECRYPT(password,'windi'),"+
                        ...
                        "ringkasan_tindakan,lama_pelayanan_pasien,surat_sakit_pihak_2,tagihan_hutang_obat,referensi_mobilejkn_bpjs,batal_pendaftaran_mobilejkn_bpjs,labu_darah,oksigen from user order by AES_DECRYPT(id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
                    ...
                    try {
                        if(rs.getString(1).toLowerCase().contains(TCari.getText().toLowerCase())||
                            ...
                            tabMode.addRow(new Object[]{rs.getString(1),
                               ...
                               rs.getBoolean("labu_darah"),
                               rs.getBoolean("oksigen")
                            });
                        }   
                    } catch (Exception e) {
                        tabMode.addRow(new Object[]{rs.getString(1),
