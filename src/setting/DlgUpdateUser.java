    ...
    */
    ...
    private void tampil(String user) {        
        try{    
            ...
            ps=koneksi.prepareStatement("select penyakit, obat_penyakit, dokter, jadwal_praktek, petugas, pasien, registrasi, tindakan_ralan,"+
                        ...
                        "ringkasan_tindakan,lama_pelayanan_pasien,surat_sakit_pihak_2,tagihan_hutang_obat,referensi_mobilejkn_bpjs,batal_pendaftaran_mobilejkn_bpjs,labu_darah,oksigen from user where id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                while(rs.next()){
                    ...
                    if("[A]Transfusi Darah".toLowerCase().contains(TCari.getText().toLowerCase())){
                        tabMode.addRow(new Object[]{false,"[A]Labu Darah",rs.getBoolean("labu_darah")});
                    }
                    if("[A]Oksigen".toLowerCase().contains(TCari.getText().toLowerCase())){
                        tabMode.addRow(new Object[]{false,"[A]Oksigen ",rs.getBoolean("oksigen")});
                    }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[A]Transfusi Darah".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","labu_darah='"+tbUser.getValueAt(i,2).toString()+"'");
            }
            if("[A]Oksigen".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","oksigen='"+tbUser.getValueAt(i,2).toString()+"'");
            }
        }
