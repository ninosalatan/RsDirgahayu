...
public class InventorySuratPemesanan extends javax.swing.JDialog {
    ...
    public InventorySuratPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
                ...
                "Disk(%)","Diskon(Rp)","Total","Jml.Stok","Isi","Isi Besar","Keterangan"
            }){
             @Override public boolean isCellEditable(int rowIndex, int colIndex){
               boolean a = false;
               if ((colIndex==0)||(colIndex==5)||(colIndex==7)||(colIndex==8)||(colIndex==13)) {
                   a=true;
               }
               return a;
             }
             ...
             Class[] types = new Class[] {
                ...
                java.lang.Double.class,java.lang.Double.class,java.lang.Double.class ,java.lang.Double.class ,java.lang.Double.class,java.lang.String.class 
             };
             @Override
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
                    ...
                    for(i=0;i<jml;i++){
                        try {
                            if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                                if(Sequel.menyimpantf2("detail_surat_pemesanan_medis","?,?,?,?,?,?,?,?,?,?,?","Transaksi Pemesanan",11,new String[]{
                                    NoPemesanan.getText(),
    ...
    */
    ...
    private void tampil() {
        ...
        for(i=0;i<row;i++){
            try {
                if(Double.parseDouble(tbDokter.getValueAt(i,0).toString())>0){
                    ...
                    keterangan[index]=tbDokter.getValueAt(i,13).toString();
                    index++;
                }
            } catch (Exception e) {
            }
        ...
        for(i=0;i<jml;i++){
            tabMode.addRow(new Object[]{jumlah[i],satuanbeli[i],kodebarang[i],namabarang[i],satuan[i],harga[i],subtotal[i],diskon[i],besardiskon[i],jmltotal[i],jmlstok[i],isi[i],isibesar[i],keterangan[i]});
        }
