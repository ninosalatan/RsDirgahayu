...
public class DlgPermintaan extends javax.swing.JDialog {
    ...
    public DlgPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul={"Jml","Kode Barang","Nama Barang","Satuan","Jenis Obat","Kategori","Golongan","Keterangan","Nominal"};
        ...
        if(tbDokter.getRowCount()!=0){
            if((evt.getKeyCode()==KeyEvent.VK_ENTER)||(evt.getKeyCode()==KeyEvent.VK_UP)||(evt.getKeyCode()==KeyEvent.VK_DOWN)){
                ...
                if(tbDokter.getSelectedColumn()==0) {
                	double nominal=Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString())*Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(),8).toString());
                	tbDokter.setValueAt(String.valueOf(nominal), tbDokter.getSelectedRow(), 8);
                }
    ...
    private void tampil() {
        try{
            ...
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                	double nominal=Double.parseDouble(rs.getString("h_beli"))*10/100+rs.getDouble("h_beli");
                    tabMode.addRow(new Object[]{
                        "",rs.getString(1),rs.getString(2),rs.getString(3),
                        rs.getString(4),rs.getString(5),rs.getString(6),"",nominal
                    });
                    iyem=iyem+"{\"KodeBarang\":\""+rs.getString(1)+"\",\"NamaBarang\":\""+rs.getString(2).replaceAll("\"","")+"\",\"Satuan\":\""+rs.getString(3)+"\",\"JenisObat\":\""+rs.getString(4)+"\",\"Kategori\":\""+rs.getString(5)+"\",\"Golongan\":\""+rs.getString(6)+"\",\"Golongan\":\""+String.valueOf(nominal)+"\"},";
                } 
            } catch (Exception e) {
                System.out.println("Notifikasi : "+e);
            } finally{
        ...
        try{
            ...
            for(i=0;i<row;i++){
                if(!tbDokter.getValueAt(i,0).toString().equals("")){
                    ...
                    nominal[index]=tbDokter.getValueAt(i,8).toString();
                    index++;
                }
            }
            Valid.tabelKosong(tabMode);
            for(i=0;i<jml;i++){
                tabMode.addRow(new Object[]{jumlah[i],kodebarang[i],namabarang[i],satuan[i],jenis[i],kategori[i],golongan[i],keterangan[i],nominal[i]});
            }
