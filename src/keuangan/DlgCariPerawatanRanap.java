...
import dirgahayu.DlgKtgPerawatan;
                ...
                switch (pilihtable) {
                    ...
                    case "rawat_inap_drpr":
                        while(rs.next()){
                        	...
                        	String nmKelas=Sequel.cariIsi("Select nm_kelas from kelas_kamar where idkelas_kamar=?",rs.getString("kelas"));
//                            if(rs.getDouble("total_byrdrpr")>0){
                                tabMode.addRow(new Object[] {
                                    false,rs.getString(1),rs.getString(2),rs.getString(3),
                                    price,rs.getDouble("material"),
                                    rs.getDouble("bhp"),price,
                                    price,rs.getDouble("kso"),
                                    rs.getDouble("menejemen"),nmKelas
                                });
                                iyem=iyem+"{\"Kode\":\""+rs.getString(1)+"\",\"NamaPerawatan\":\""+rs.getString(2).replaceAll("\"","")+"\",\"KategoriPerawatan\":\""+rs.getString(3)+"\",\"Tarif\":\""+price+"\",\"BagianRS\":\""+rs.getString("material")+"\",\"BHP\":\""+rs.getString("bhp")+"\",\"JMDokter\":\""+rs.getString("tarif_tindakandr")+"\",\"JMPerawat\":\""+rs.getString("tarif_tindakanpr")+"\",\"KSO\":\""+rs.getString("kso")+"\",\"Menejemen\":\""+rs.getString("menejemen")+"\",\"Kelas\":\""+rs.getString("kelas")+"\"},";
//                            }                        
                        }   
                        break;
                }
    ...
    public void setNoRm(String norwt,String pilihtable,Date tanggal,String jam,String menit,String detik,boolean status,String pasien) {
        ...
        if(!norawatibu.equals("")){
            ...
            this.kelas=Sequel.cariIsi(
                    "select kelas_kamar.idkelas_kamar from kamar inner join kamar_inap "+
                    "on kamar.kd_kamar=kamar_inap.kd_kamar Inner join kelas_kamar on kelas_kamar.nm_kelas=kamar.kelas where no_rawat=? "+
