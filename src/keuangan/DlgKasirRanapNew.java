...
public class DlgKasirRanapNew extends javax.swing.JDialog {
	...
	public static void main(String[] args) {
		java.awt.EventQueue.invokeLater(() -> {
			DlgKasirRanapNew dialog = new DlgKasirRanapNew(new javax.swing.JFrame(), true);
	...
	private TextBox tDeposit;

	public DlgKasirRanapNew(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		setUndecorated(true);
...
//		panelDataTableBilling.add(tbBilling);
		...
		lblDeposit.setBounds(657, 70, 96, 21);
		panelDataTotalBilling.add(lblDeposit);
		
		 tDeposit = new TextBox();
		tDeposit.setBounds(763, 65, 198, 23);
		tDeposit.setEditable(false);
		panelDataTotalBilling.add(tDeposit);
...
//			                            tPasien.setText(reg.getTable().getValueAt(reg.getTable().getSelectedRow(),7).toString());
		...
		jLabelCaraBayar.setBounds(10, 176, 64, 23);
		...
		rdbtnCash.setBounds(131, 176, 71, 23);
		panelGlassTop.add(rdbtnCash);

		rdbtnPiutang = new JRadioButton("Piutang");
		rdbtnPiutang.setBounds(220, 176, 78, 23);
		...
		jLabelAkunPenerimaan.setBounds(421, 143, 64, 23);
		panelGlassTop.add(jLabelAkunPenerimaan);

		comboBox = new widget.ComboBox();
		comboBox.setEditable(false);
		comboBox.enable(false);
		comboBox.setBounds(537, 148, 167, 22);
		...
		Label jLabelDjamin = new Label();
		jLabelDjamin.setHorizontalAlignment(SwingConstants.LEFT);
		jLabelDjamin.setBounds(421, 176, 97, 23);
		jLabelDjamin.setText("Kelas Dijamin :");
		panelGlassTop.add(jLabelDjamin);
		
		cmbKelas = new ComboBox();
		cmbKelas.setBounds(537, 181, 167, 22);
		sekuel Sequel =new sekuel();
		Sequel.addItemCombo("Select nm_kelas from kelas_kamar order by idkelas_kamar DESC", cmbKelas);
		panelGlassTop.add(cmbKelas);
...
//			
	...
	public void SetNoRm(String noRawat) {
		...
		tpoli.setText(Sequel.cariIsi(
				"select nm_bangsal from kamar_inap\r\n"
...
//		tabelModelBilling.setRowCount(0);
	...
	public void getDeposito() {
		sekuel Sequel = new sekuel();
		tDeposit.setText(Sequel.cariIsi("select besar_deposit from deposit where no_rawat=?",tNoRw.getText()));
	}
	public void getTransferBill() {
		sekuel Sequel = new sekuel();
		if(Sequel.cariIsi("select status_bayar from reg_periksa where no_rawat=?",tNoRw.getText()).equals("Transfer")) {
			getDataTindakanRalan();
			getDataObatRalan();
			
		}
	}
	public void getDataTindakanRalan() {
		sekuel Sequel = new sekuel();
...
//			System.out.println("Tidak Punya Tindakan:" + tNoRw.getText());
	...
	public void getDataObatRacikRalan() {
		sekuel Sequel = new sekuel();
...
//					double JmlBayarObat = hargaObat;
	...
	private void getBHPLabRalan(String noOrder) {
		Connection koneksi = koneksiDB.condb();
		ResultSet rs;
	...
	private void getDataLaborat() {
		...
		if (Sequel.cariInteger("Select count(no_rawat) from permintaan_lab where no_rawat=?", tNoRw.getText()) > 0) {
			...
			try {
				...
				while (rs != null && rs.next()) {
					...
					getBHPLabRalan(rs.getString("noorder"));
					tabelRowLaborat++;
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
		...
		getDataLaboratPaRalan();
	}

	private void getDataLaboratPaRalan() {
		// TODO Auto-generated method stub
		sekuel Sequel = new sekuel();
	...
	private void getDataNursing() {
		...
		Connection koneksi = koneksiDB.condb();
		sekuel Sequel = new sekuel();
		if (Sequel.cariInteger("Select count(no_rawat) from rawat_inap_drpr where no_rawat=?",
				tNoRw.getText()) > 0) {
			tabelModelBilling.addRow(new Object[] { "  Biaya Perawatan Rawat Inap" });
			try {
				psRoom = koneksi.prepareStatement(
						"select nm_perawatan,rawat_inap_drpr.material,rawat_inap_drpr.tarif_tindakandr,rawat_inap_drpr.kd_jenis_prw from rawat_inap_drpr\r\n"
						+ "inner join jns_perawatan_inap on jns_perawatan_inap.kd_jenis_prw=rawat_inap_drpr.kd_jenis_prw\r\n"
						+ "where rawat_inap_drpr.no_rawat=? group by rawat_inap_drpr.kd_jenis_prw");
				psRoom.setString(1, tNoRw.getText());
				rs = psRoom.executeQuery();
				String StatusBayar = "Belum Lunas";
				while (rs.next()) {
					Integer jml=Sequel.cariInteger("select count(kd_jenis_prw) from rawat_inap_drpr where no_rawat='"+tNoRw.getText()+"' and kd_jenis_prw='"+rs.getString("kd_jenis_prw")+"'");
					double price=rs.getDouble("material")+rs.getDouble("tarif_tindakandr");
					double sumPrice=jml*price;
					tabelModelBilling.addRow(new Object[] { rs.getString("nm_perawatan"),jml,
							price, 0, 0, sumPrice, StatusBayar,
							tNoRw.getText(), "rawat_inap_drpr" });
				}
	...
	private void getDataRooms() {
		...
		if (Sequel.cariInteger("Select count(no_rawat) from kamar_inap where no_rawat=?",
				tNoRw.getText()) > 0) {
			tabelModelBilling.addRow(new Object[] { "  Biaya Ruangan" });
			try {
				psRoom = koneksi.prepareStatement(
						"select nm_bangsal,kamar_inap.trf_kamar,lama from kamar_inap\r\n"
						+ "inner join kamar on kamar.kd_kamar=kamar_inap.kd_kamar\r\n"
						+ "inner join bangsal on kamar.kd_bangsal=bangsal.kd_bangsal\r\n"
						+ "where kamar_inap.no_rawat=?");
				psRoom.setString(1, tNoRw.getText());
				...
				String StatusBayar = "Belum Lunas";
				while (rs.next()) {
					double sumPrice=rs.getDouble("lama")*rs.getDouble("trf_kamar");
					tabelModelBilling.addRow(new Object[] { rs.getString("nm_bangsal"),rs.getInt("lama"),
							rs.getDouble("trf_kamar"), 0, 0, sumPrice, StatusBayar,
							tNoRw.getText(), "kamar_inap" });
				}
	...
	public void getDataObat() {
		...
		if (Sequel.cariInteger(
			...
			String query = "select resep_obat.no_resep,databarang.kode_brng,databarang.nama_brng,resep_dokter.jml,databarang.kode_sat,databarang.dasar,"
					...
					+ "inner join resep_dokter on resep_dokter.no_resep=resep_obat.no_resep\r\n"
					+ "inner join databarang on resep_dokter.kode_brng=databarang.kode_brng \r\n"
					+ "where resep_obat.status='ranap' and resep_obat.tgl_perawatan between '" + tanggal + "' and '"
					+ tanggal + "' " + "and  resep_obat.jam_peresepan!=resep_obat.jam\r\n"
					+ " and reg_periksa.no_rawat='" + tNoRw.getText() + "'"
					+ "order by resep_obat.tgl_perawatan desc,resep_obat.jam desc";
			// System.out.println(query);
	...
	public void getDataObatRacik() {
		...
		if (Sequel.cariInteger("Select count(no_rawat) from obat_racikan where no_rawat=? ", tNoRw.getText()) > 0) {
			...
			String query = "select nama_racik,nm_racik,jml_dr,resep_obat.jam,status_bayar,no_resep from obat_racikan "
					+ "Inner join resep_obat on obat_racikan.no_rawat=resep_obat.no_rawat and resep_obat.tgl_perawatan=obat_racikan.tgl_perawatan and resep_obat.jam=obat_racikan.jam "
					+ "Inner join metode_racik on metode_racik.kd_racik=obat_racikan.kd_racik "
					+ "where resep_obat.no_rawat='" + tNoRw.getText() + "' and resep_obat.status='ranap'" + "group by resep_obat.jam ";
			// System.out.println(query);
			try {
				rs = koneksi.prepareStatement(query).executeQuery();
				while (rs != null && rs.next()) {
