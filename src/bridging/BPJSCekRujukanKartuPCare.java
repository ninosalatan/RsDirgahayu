...
public final class BPJSCekRujukanKartuPCare extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        btnNoRujuk=new widget.Button();
        btnNoRujuk.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        btnNoRujuk.setMnemonic('4');
        btnNoRujuk.setToolTipText("ALt+4");
        btnNoRujuk.setName("btnNoRujuk"); // NOI18N
        btnNoRujuk.addActionListener(new java.awt.event.ActionListener() {
            @Override
			public void actionPerformed(java.awt.event.ActionEvent evt) {
                noRujukanKeyPressed(evt);
            }
        });
        FormKelengkapanSEP.add(btnNoRujuk);
        btnNoRujuk.setBounds(755, 25, 28, 23);
        ...
        internalFrame1.setMinimumSize(new Dimension(1000, 337));
    ...
    protected void noRujukanKeyPressed(ActionEvent evt) {
    ...
    public void tampil(String nomorrujukan) {
	    ...
	    root = mapper.readTree(api.getRest().exchange(URL, HttpMethod.GET, requestEntity, String.class).getBody());
            ...
            if(nameNode.path("code").asText().equals("200")){
                ...
                Valid.SetTgl(TanggalRujuk, response.path("tglKunjungan").asText().toString());
                countgetDays(TanggalRujuk.getSelectedItem().toString());
...
//            isNumber();
    ...
    private void isNumber(){    
            ...
            switch (URUTNOREG) {
                ...
                case "dokter + poli":  
                    if(Sequel.cariInteger("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'")>=
                        ...
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",kdpoli.getText() + "-" ,1,TNoReg);
                    }else{
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from reg_periksa where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tgl_registrasi='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",kdpoli.getText() + "-" ,1,TNoReg);
                    }                    
                    break;
                default:
                    if(Sequel.cariInteger("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'")>=
        ...
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_rawat,6),signed)),0) from reg_periksa where tgl_registrasi='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"").substring(0,10)+"' ",dateformat.format(TanggalSEP.getDate())+ "/" + kddokter.getText() + "/",4,TNoRw);                              
    }
...
//        System.out.println(NoPeserta);;
    ...
    public void countgetDays(String tglRujuk) {
     SimpleDateFormat myFormat = new SimpleDateFormat("dd-MM-yyyy");
   	 String date1 = tglRujuk;
   	 String date2 = "27-09-2021";
   	 try {
   		int diffDays = 0;
...
//	    System.out.println("Number of Days between dates: "+diffDays);
        if(diffDays>90) {
