...
import dirgahayu.DlgPilihanCetakDokumen;
...
//        dpjp.setVisible(true);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        else if(NoRujukan.getText().trim().equals("")&&JenisPelayanan.getSelectedIndex()==1){
    ...
    */
    ...
    private void isNumber(){    
            ...
            switch (URUTNOREG) {
                ...
                case "dokter + poli":  
                    if(Sequel.cariInteger("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'")>=
                        ...
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",kdpoli.getText()+"-",3,TNoReg);
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",Valid.SetTglNospce(TanggalSEP.getSelectedItem()+"")+"",3,NoRujukan);
                    }else{
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from reg_periksa where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tgl_registrasi='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",kdpoli.getText()+"-",3,TNoReg);
                        Valid.autoNomer3("select ifnull(MAX(CONVERT(no_reg,signed)),0) from booking_registrasi where kd_dokter='"+kddokter.getText()+"' and kd_poli='"+kdpoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"")+"'",Valid.SetTglNospce(TanggalSEP.getSelectedItem()+"")+"",3,NoRujukan);
                    }                    
                    break;
                default:
        ...
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_rawat,6),signed)),0) from reg_periksa where tgl_registrasi='"+Valid.SetTgl(TanggalSEP.getSelectedItem()+"").substring(0,10)+"' ",dateformat.format(TanggalSEP.getDate())+ "/" + kddokter.getText() + "/",6,TNoRw);                              
    }
...
//                        pilihan.tampil();
                         ...
                         param.put("noreg",TNoReg.getText());
...
//                        pilihan.setVisible(true);
    ...
    private void inputRegistrasi(){
        ...
        if(JenisPelayanan.getSelectedIndex()==1){
            ...
            if(Sequel.menyimpantf2("reg_periksa","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",21,
                    ...
                    klg,TBiaya.getText(),"Belum",statuspasien,"Ralan",Kdpnj.getText(),umur,sttsumur,"Belum Bayar",status,NoKartu.getText(),NoRujukan.getText()
                })==true){
                    UpdateUmur();
                    if(nosisrute.equals("")){
            ...
            isNumber();
            Sequel.menyimpan("poliklinik","?,?,?,?,?",5,new String[]{"IGD","Unit IGD","0","0","1"});
            if(Sequel.menyimpantf2("reg_periksa","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",21,
                    new String[]{TNoReg.getText(),TNoRw.getText(),Valid.SetTgl(TanggalSEP.getSelectedItem()+""),TanggalSEP.getSelectedItem().toString().substring(11,19),
                    kddokter.getText(),TNo.getText(),"IGD",Saudara.getText(),AlamatPj.getText()+", "+KelurahanPj.getText()+", "+KecamatanPj.getText()+", "+KabupatenPj.getText(),
                    klg,Sequel.cariIsi("select registrasilama from poliklinik where kd_poli='IGDK'"),"Belum",statuspasien,"Ralan",Kdpnj.getText(),umur,sttsumur,"Belum Bayar",status,NoKartu.getText(),NoRujukan.getText()
                })==true){
                    UpdateUmur();
                    if(nosisrute.equals("")){
                        Sequel.menyimpan2("rujuk_masuk","'"+TNoRw.getText()+"','"+NmPpkRujukan.getText()+"','"+Kabupaten.getText()+"','"+NoRujukan.getText()+"','0','"+NmPPK.getText()+"','"+KdPenyakit.getText()+"','-','-','"+NoBalasan.getText()+"'","No.Rujuk");             
                    ...
                    if(Sequel.menyimpantf2("kamar_inap","'"+TNoRw.getText()+"','"+KdPoli.getText()+"','"+TBiaya.getText()+"','"+
                            ...
                            "'00:00:00','"+jmlhari+"','"+biaya+"','-','Belum Lunas','"+Kelas.getSelectedItem().toString()+"'","No.Rawat"
                        )==true){
    ...
    public void isIGD(String status,String kdpolis,String namapoli) {
    	KdPoli.setText(Sequel.cariIsi("select kd_poli_bpjs from maping_poli_bpjs where kd_poli_bpjs='"+kdpolis+"'"));
        kdpoli.setText(Sequel.cariIsi("select kd_poli_rs from maping_poli_bpjs where kd_poli_bpjs='"+kdpolis+"'"));
