...
public final class DlgPermintaanRadiologi extends javax.swing.JDialog {
	...
	private void initComponents() {
		...
		internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				...
				"::[ Input Data Permintaan Radiologi ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
...
//		BtnSimpan.setText("Simpan");
	...
	private PreparedStatement pspemeriksaanEdit;
...
//				System.out.println(BtnSimpan.getText());
				if (!BtnSimpan.getText().equals("Ubah")) {
					if (cara_bayar_radiologi.equals("Yes") && kelas_radiologi.equals("No")) {
						...
						// System.out.println("Haloo "+pspemeriksaan3);
					...
					pspemeriksaanEdit = koneksi.prepareStatement(
							"select jns_perawatan_radiologi.kd_jenis_prw,jns_perawatan_radiologi.nm_perawatan,jns_perawatan_radiologi.total_byr,"
									+ "jns_perawatan_radiologi.bagian_rs,jns_perawatan_radiologi.bhp,jns_perawatan_radiologi.tarif_perujuk,"
									+ "jns_perawatan_radiologi.tarif_tindakan_dokter,jns_perawatan_radiologi.tarif_tindakan_petugas,"
									+ "jns_perawatan_radiologi.kso,jns_perawatan_radiologi.menejemen,penjab.png_jawab,IF(permintaan_pemeriksaan_radiologi.noorder IS NULL,0,1) as noPermintaan  "
									+ "from jns_perawatan_radiologi inner join penjab on penjab.kd_pj=jns_perawatan_radiologi.kd_pj left join permintaan_pemeriksaan_radiologi on permintaan_pemeriksaan_radiologi.kd_jenis_prw=jns_perawatan_radiologi.kd_jenis_prw and permintaan_pemeriksaan_radiologi.noorder='"
									+ TNoPermintaan.getText() + "' where "
									+ " jns_perawatan_radiologi.status='1' and (jns_perawatan_radiologi.kd_pj=? or jns_perawatan_radiologi.kd_pj='-') and (jns_perawatan_radiologi.kelas=? or jns_perawatan_radiologi.kelas='-') and jns_perawatan_radiologi.kd_jenis_prw like ? or "
									+ " jns_perawatan_radiologi.status='1' and (jns_perawatan_radiologi.kd_pj=? or jns_perawatan_radiologi.kd_pj='-') and (jns_perawatan_radiologi.kelas=? or jns_perawatan_radiologi.kelas='-') and jns_perawatan_radiologi.nm_perawatan like ? "
									+ "order by jns_perawatan_radiologi.kd_jenis_prw");

					pspemeriksaanEdit.setString(1, Penjab.getText().trim());
					pspemeriksaanEdit.setString(2, kelas.trim());
					pspemeriksaanEdit.setString(3, "%" + TCariPeriksa.getText().trim() + "%");
					pspemeriksaanEdit.setString(4, Penjab.getText().trim());
					pspemeriksaanEdit.setString(5, kelas.trim());
					pspemeriksaanEdit.setString(6, "%" + TCariPeriksa.getText().trim() + "%");
//					 System.out.println(pspemeriksaan3);
					rs = pspemeriksaanEdit.executeQuery();
				}

				while (rs.next()) {
					if (BtnSimpan.getText().trim().equals("Ubah")) {
						if (rs.getInt("noPermintaan") == 1) {
							ischeck = true;
					...
					tabMode.addRow(new Object[] { ischeck, rs.getString(1), rs.getString(2) });
				}
			} catch (Exception e) {
	...
	private void isRawat() {
		...
		IscheckNoPermintaan(TNoPermintaan.getText());
	}
	...
	public void setTNoPermintaan(String noPermintaan) {
		TNoPermintaan.setText(noPermintaan);
		InformasiTambahan.setText(
				Sequel.cariIsi("Select informasi_tambahan from permintaan_radiologi where noorder=?", noPermintaan));
		DiagnosisKlinis.setText(
				Sequel.cariIsi("Select diagnosa_klinis from permintaan_radiologi where noorder=?", noPermintaan));

	}

	public void IscheckNoPermintaan(String noPermintaan) {
		if (Sequel.cariInteger("select count(*) from permintaan_radiologi where noorder='" + noPermintaan + "'") == 0) {
			BtnSimpan.setText("Simpan");
	...
	private void btnUbahActionPerformed(java.awt.event.ActionEvent evt) {
		...
		int reply = JOptionPane.showConfirmDialog(rootPane, "Apakah Anda bener akan mengubah data ini?", "Konfirmasi",
				JOptionPane.YES_NO_OPTION);
		if (reply == JOptionPane.YES_OPTION) {
			if (tbPemeriksaan.getSelectedRow() != -1) {
				ChkJln.setSelected(false);
				Sequel.meghapus("permintaan_pemeriksaan_radiologi", "noorder", TNoPermintaan.getText());
				try {
				...
				JOptionPane.showMessageDialog(null, "Silahkan Memilih data");
			}
		}
	}
