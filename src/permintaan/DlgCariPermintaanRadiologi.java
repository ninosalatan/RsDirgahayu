...
import dirgahayu.DlgCariPoli;
import dirgahayu.DlgPeriksaRadiologi;
import dirgahayu.DlgRegRadiologi;
...
public class DlgCariPermintaanRadiologi extends javax.swing.JDialog {
	...
	public DlgCariPermintaanRadiologi(java.awt.Frame parent, boolean modal) {
		...
		tabMode = new DefaultTableModel(null,
				new Object[] { "No.Permintaan", "No.Rawat", "Pasien", "Permintaan", "Jam", "Sampel", "Jam", "Hasil",
						...
						"Diagnosis Klinis", "Status Bayar","No RM","Penjamin" }) {
			@Override
	...
	private void initComponents() {
		...
		BtnHasil.setBounds(120, 10, 100, 30);
		BtnSampel = new widget.Button();
		BtnSampel.setBounds(230, 10, 100, 30);
		BtnAll = new widget.Button();
		BtnAll.setBounds(340, 10, 100, 30);
		BtnPrint = new widget.Button();
		BtnPrint.setBounds(450, 10, 122, 30);
		jLabel10 = new widget.Label();
		jLabel10.setBounds(591, 14, 60, 23);
		LCount = new widget.Label();
		LCount.setBounds(680, 14, 53, 23);
		BtnKeluar = new widget.Button();
		BtnKeluar.setBounds(724, 10, 78, 30);
		...
		MnUbahWO = new javax.swing.JMenuItem();
		MnUbahWO.setBackground(new java.awt.Color(255, 255, 254));
		MnUbahWO.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
		MnUbahWO.setForeground(new java.awt.Color(50, 50, 50));
		MnUbahWO.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png")));
		MnUbahWO.setText("Ubah WO");
		MnUbahWO.setName("MnUbahWO"); // NOI18N
		MnUbahWO.setPreferredSize(new java.awt.Dimension(200, 28));
		MnUbahWO.addActionListener(new java.awt.event.ActionListener() {
			@Override
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				MnUbahWOActionPerformed(evt);
			}
		});
		jPopupMenu1.add(MnUbahWO);
		...
		internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)),
				"::[ Data Permintaan Radiologi ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
				javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11),
				new java.awt.Color(50, 50, 50))); // NOI18N
		...
		BtnPrint.addActionListener(new java.awt.event.ActionListener() {
			@Override
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				BtnNotaJasperActionPerformed(evt);
			}
		});
		...
		TabRawatJalan.addTab("Daftar Pemeriksaan", scrollPane2);
	...
	private void BtnNotaJasperActionPerformed(java.awt.event.ActionEvent evt) {
		if (TabPilihRawat.getSelectedIndex() == 0) {		
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					String noorder=tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim();
					String dates = tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 3).toString().trim()+" "+tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 4).toString().trim();
					...
					param.put("nm_pasien",tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 2).toString().trim());					
					param.put("nm_dockter", tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 10).toString().trim());	
					if( tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 11)
							.toString().equals("Radiologi Luar")) {
						System.out.println("s");
						param.put("tgl_lahir", Sequel.cariString("select tgl_lahir from reg_noRm where no_reg='"+noorder+"'"));
					...
					param.put("no_rm",tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 15).toString().trim());					
					param.put("no_rawat", tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString().trim());	
					Valid.MyReportqry("notaRadiologi.jasper", "report", "::[ Billing Radiologi & Pemeriksaan ]::",
							"select ROW_NUMBER() OVER(PARTITION BY permintaan_pemeriksaan_radiologi.noorder) as no,jns_perawatan_radiologi.nm_perawatan,jns_perawatan_radiologi.total_byr, "
									+ "reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,pasien.jk,pasien.umur,pasien.alamat,poliklinik.nm_poli,reg_periksa.tgl_registrasi,penjab.png_jawab "
									+ "from permintaan_pemeriksaan_radiologi INNER JOIN jns_perawatan_radiologi inner join permintaan_radiologi inner JOIN reg_periksa INNER JOIN pasien inner join penjab inner join poliklinik "
									+ "on permintaan_pemeriksaan_radiologi.kd_jenis_prw=jns_perawatan_radiologi.kd_jenis_prw and permintaan_pemeriksaan_radiologi.noorder = permintaan_radiologi.noorder "
									+ "and permintaan_radiologi.no_rawat=reg_periksa.no_rawat and reg_periksa.no_rkm_medis=pasien.no_rkm_medis and reg_periksa.kd_poli= poliklinik.kd_poli and reg_periksa.kd_pj=penjab.kd_pj "
			...
			if (TabRawatJalan.getSelectedIndex() == 0) {
				if (tabMode.getRowCount() == 0) {
					...
					for (i = 0; i < row; i++) {
						...
						Sequel.menyimpan("temporary_permintaan_radiologi", "'0','" + tabMode.getValueAt(i, 0).toString()
				...
				if (tabMode2.getRowCount() == 0) {
					...
					for (i = 0; i < row; i++) {
						...
						Sequel.menyimpan("temporary_permintaan_radiologi", "'0','"
								+ tabMode2.getValueAt(i, 0).toString() + "','" + tabMode2.getValueAt(i, 1).toString()
								+ "','" + tabMode2.getValueAt(i, 2).toString() + "','"
								+ tabMode2.getValueAt(i, 3).toString() + "','" + tabMode2.getValueAt(i, 4).toString()
								+ "','" + tabMode2.getValueAt(i, 5).toString() + "','" + tglsampel + "','"
								+ tabMode2.getValueAt(i, 7).toString() + "','" + tglhasil + "','"
								+ tabMode2.getValueAt(i, 9).toString() + "','" + tabMode2.getValueAt(i, 10).toString()
					...
					param.put("emailrs", akses.getemailrs());
					param.put("logo", Sequel.cariGambar("select logo from setting"));
					Valid.MyReport("rptLapPermintaanRadiologi2.jasper", "report",
							"::[ Data Detail Permintaan Radiologi ]::", param);
				}
			}
		} else if (TabPilihRawat.getSelectedIndex() == 1) {
			if (TabRawatInap.getSelectedIndex() == 0) {
				if (tabMode3.getRowCount() == 0) {
					...
					for (i = 0; i < row; i++) {
						...
						Sequel.menyimpan("temporary_permintaan_radiologi", "'0','"
								+ tabMode3.getValueAt(i, 0).toString() + "','" + tabMode3.getValueAt(i, 1).toString()
								+ "','" + tabMode3.getValueAt(i, 2).toString() + "','"
								+ tabMode3.getValueAt(i, 3).toString() + "','" + tabMode3.getValueAt(i, 4).toString()
								+ "','" + tglsampel + "','" + tabMode3.getValueAt(i, 6).toString() + "','" + tglhasil
								+ "','" + tabMode3.getValueAt(i, 8).toString() + "','"
								+ tabMode3.getValueAt(i, 9).toString() + "','" + tabMode3.getValueAt(i, 10).toString()
				...
				if (tabMode4.getRowCount() == 0) {
					...
					for (i = 0; i < row; i++) {
						...
						Sequel.menyimpan("temporary_permintaan_radiologi", "'0','"
								+ tabMode4.getValueAt(i, 0).toString() + "','" + tabMode4.getValueAt(i, 1).toString()
								+ "','" + tabMode4.getValueAt(i, 2).toString() + "','"
								+ tabMode4.getValueAt(i, 3).toString() + "','" + tabMode4.getValueAt(i, 4).toString()
								+ "','" + tabMode4.getValueAt(i, 5).toString() + "','" + tglsampel + "','"
								+ tabMode4.getValueAt(i, 7).toString() + "','" + tglhasil + "','"
								+ tabMode4.getValueAt(i, 9).toString() + "','" + tabMode4.getValueAt(i, 10).toString()
					...
					param.put("emailrs", akses.getemailrs());
					param.put("logo", Sequel.cariGambar("select logo from setting"));
					Valid.MyReport("rptLapPermintaanRadiologi4.jasper", "report",
							"::[ Data Detail Permintaan Radiologi ]::", param);
				}
			}
		}
	...
	private void BtnPrintKeyPressed(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_BtnPrintKeyPressed
		if (evt.getKeyCode() == KeyEvent.VK_SPACE) {
			BtnNotaJasperActionPerformed(null);
		} else {
	...
	private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnHapusActionPerformed
		if (TabPilihRawat.getSelectedIndex() == 0) {
			if (TabRawatJalan.getSelectedIndex() == 0) {
				if (tbRadiologiRalan.getSelectedRow() != -1) {
					if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim()
							.equals("")) {
						Valid.textKosong(TCari, "No.Permintaan");
					} else {
						if (Sequel.cariInteger(
								"select count(noorder) from permintaan_pemeriksaan_radiologi where stts_bayar='Sudah' and noorder=?",
								tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString()) > 0) {
							JOptionPane.showMessageDialog(null,
			...
			if (TabRawatInap.getSelectedIndex() == 0) {
				if (tbRadiologiRanap.getSelectedRow() != -1) {
					if (tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString().trim()
							.equals("")) {
						Valid.textKosong(TCari, "No.Permintaan");
					} else {
						if (Sequel.cariInteger(
								"select count(noorder) from permintaan_pemeriksaan_radiologi where stts_bayar='Sudah' and noorder=?",
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString()) > 0) {
							JOptionPane.showMessageDialog(null,
									"Maaf, Tidak boleh dihapus karena sudah ada tindakan yang sudah dibayar.\nSilahkan hubungi kasir...!!!!");
						} else {
							Sequel.meghapus("permintaan_radiologi", "noorder",
		...
		if (TabPilihRawat.getSelectedIndex() == 0) {
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				...
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
						...
						try {
							...
							while (rs2.next()) {
								Sequel.menyimpan("temporary_permintaan_radiologi",
										"?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",
										38,
										new String[] { "0", rs2.getString("kd_jenis_prw"),
					...
					param.put("noperiksa",
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString());
					norm = Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=? ",
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString());
					param.put("norm", norm);
					...
					param.put("pengirim",
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 10).toString());
					param.put("informasitambahan",
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 12).toString());
					param.put("diagnosa",
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 13).toString());
					param.put("tanggal", Valid
							.SetTgl3(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 3).toString()));
					...
					param.put("finger", Sequel.cariIsi(
							...
							tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 9).toString()));
			...
			if (tbRadiologiRanap.getSelectedRow() != -1) {
				...
				if (tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					param.put("noperiksa",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString());
					norm = Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=? ",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 1).toString());
					param.put("norm", norm);
					param.put("pekerjaan", Sequel.cariIsi("select pekerjaan from pasien where no_rkm_medis=?", norm));
					param.put("noktp", Sequel.cariIsi("select no_ktp from pasien where no_rkm_medis=?", norm));
					param.put("namapasien", Sequel.cariIsi("select nm_pasien from pasien where no_rkm_medis=? ", norm));
					...
					param.put("lahir", Sequel.cariIsi(
							"select DATE_FORMAT(tgl_lahir,'%d-%m-%Y') from pasien where no_rkm_medis=? ", norm));
					param.put("pengirim",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 10).toString());
					param.put("informasitambahan",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 12).toString());
					param.put("diagnosa",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 13).toString());
					param.put("tanggal", Valid
							.SetTgl3(tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 3).toString()));
					param.put("alamat", Sequel.cariIsi(
							"select concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) as alamat from pasien inner join kelurahan inner join kecamatan inner join kabupaten on pasien.kd_kel=kelurahan.kd_kel and pasien.kd_kec=kecamatan.kd_kec and pasien.kd_kab=kabupaten.kd_kab where no_rkm_medis=? ",
							norm));
					...
					param.put("finger", Sequel.cariIsi(
							"select sha1(sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",
							tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 9).toString()));
			...
			if (TabRawatJalan.getSelectedIndex() == 0) {
				if (tbRadiologiRalan.getSelectedRow() != -1) {
					if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim()
						...
						if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 14).toString().trim()
								.equals("Belum Lunas")) {
							JOptionPane.showMessageDialog(null, "Maaf, Silahkan Melakukan Pembayaran terlebih dulu.!");
						} else {
							this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
							DlgPeriksaRadiologi dlgro = new DlgPeriksaRadiologi(null, false);
							String flagPermintaan = tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 11)
									.toString();
							if (!flagPermintaan.equals("Radiologi Luar")) {
								flagPermintaan = "Ralan";
							}
							dlgro.setSize(internalFrame1.getWidth() - 20, internalFrame1.getHeight() - 20);
							dlgro.setLocationRelativeTo(internalFrame1);
							dlgro.emptTeks();
							dlgro.isCek();
							dlgro.setOrder(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString(),
									tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString(),
									flagPermintaan);
			...
			if (TabRawatInap.getSelectedIndex() == 0) {
				if (tbRadiologiRanap.getSelectedRow() != -1) {
					if (tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString().trim()
						...
						dlgro.emptTeks();
						dlgro.isCek();
						dlgro.setOrder(tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString(),
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 1).toString(), "Ranap");
						dlgro.setDokterPerujuk(
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 9).toString(),
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 10).toString());
						dlgro.setVisible(true);
			...
			if (TabRawatJalan.getSelectedIndex() == 0) {
				if (tbRadiologiRalan.getSelectedRow() != -1) {
					if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim()
						...
						if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 14).toString().trim()
								.equals("Belum Lunas")&& (!tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 16).toString().trim()
								.equals("C13")||!tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 16).toString().trim()
								.equals("C14"))) {
							JOptionPane.showMessageDialog(null, "Maaf, Silahkan Melakukan Pembayaran terlebih dulu.!");
						} else {
		...
		if (TabPilihRawat.getSelectedIndex() == 0) {
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					param.put("kontakrs", akses.getkontakrs());
					param.put("emailrs", akses.getemailrs());
					Valid.MyReportqry("rptBarcodePermintaanRadiologi.jasper", "report",
							"::[ Barcode No.Permintaan Radiologi ]::",
	...
	protected void MnUbahWOActionPerformed(ActionEvent evt) {
			...
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					if (!tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 14).toString().trim()
							.equals("Belum Lunas")) {
						JOptionPane.showMessageDialog(null,
								"Maaf, Permintaan Sudah Di Sudah di tervalidasi.., Tidak dapat Merubah Pemeriksaan");
					} else {
						if(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString().trim().equals("")){
							this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
							DlgRegRadiologi radEx=new DlgRegRadiologi(null,false);
							radEx.setSize(internalFrame1.getWidth(), internalFrame1.getHeight());
							radEx.setLocationRelativeTo(internalFrame1);
							radEx.emptyTeks();
							radEx.setVisible(true);
							this.setCursor(Cursor.getDefaultCursor());
						}else {
						this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
						...
						dlgro.emptTeks();
						dlgro.isCek();
						dlgro.setTNoPermintaan(
								tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim());
						dlgro.setNoRm(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString(),
								"Ralan");
						dlgro.setVisible(true);
						this.setCursor(Cursor.getDefaultCursor());
						}
		...
		if (TabPilihRawat.getSelectedIndex() == 0) {
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					param.put("kontakrs", akses.getkontakrs());
					param.put("emailrs", akses.getemailrs());
					Valid.MyReportqry("rptBarcodePermintaanRadiologi2.jasper", "report",
							"::[ Barcode No.Permintaan Radiologi ]::",
							"select noorder from permintaan_radiologi where no_rawat='"
									+ tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString()
									+ "'",
		...
		if (TabPilihRawat.getSelectedIndex() == 0) {
			if (tbRadiologiRalan.getSelectedRow() != -1) {
				if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
					...
					if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 11).toString()
							.equals("Radiologi Luar")) {
						Sequel.mengedit("reg_noRm", "no_reg=?", "tgl_sampel=?,jam_sampel=?", 3,
								new String[] { Valid.SetTgl(TanggalPulang.getSelectedItem() + ""),
										TanggalPulang.getSelectedItem().toString().substring(11, 19),
										tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString() });
					} else {
						Sequel.mengedit("permintaan_radiologi", "noorder=?", "tgl_sampel=?,jam_sampel=?", 3,
								new String[] { Valid.SetTgl(TanggalPulang.getSelectedItem() + ""),
										TanggalPulang.getSelectedItem().toString().substring(11, 19),
										tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString() });
					}
...
//					}
				...
				try {
					...
					try {
						...
						while (rs.next()) {
							...
							koneksifuji.prepareStatement("insert into order_in values(" + "'" + Permintaan + " "
									+ JamPermintaan + "','" + norm + "','" + NoPermintaan.replaceAll("PR", "") + "','"
									+ NoRawat + "','" + Pasien.replaceAll(norm + " ", "") + "'," + "'"
									+ Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis=?", norm) + "',"
									+ "'"
									+ Sequel.cariIsi("select if(jk='L','M','F') from pasien where no_rkm_medis=?", norm)
									+ "','" + DokterPerujuk + "'," + "'"
									+ Sequel.cariIsi(
											"select dokter.nm_dokter from dokter inner join set_pjlab where set_pjlab.kd_dokterrad=dokter.kd_dokter limit 1")
									+ "'," + "'" + Ruang + "','" + rs.getString("kd_jenis_prw") + "','"
									+ rs.getString("nm_perawatan") + "','" + rs.getString("bagian_rs") + "'," + "'"
									+ rs.getString("bhp") + "','" + rs.getString("tarif_perujuk") + "','"
									+ rs.getString("tarif_tindakan_dokter") + "'," + "'"
									+ rs.getString("tarif_tindakan_petugas") + "','" + rs.getString("kso") + "','"
									+ rs.getString("menejemen") + "', " + "'" + rs.getString("total_byr") + "','"
									+ InformasiTambahan + "','" + Klinis + "','0')").executeUpdate();
						}
			...
			if (TabRawatJalan.getSelectedIndex() == 0) {
				if (tbRadiologiRalan.getSelectedRow() != -1) {
					if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim()
						...
						dlgro.emptTeks();
						dlgro.isCek();
						dlgro.setOrderFuji(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString(),
								tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString(), "Ralan");
						dlgro.setDokterPerujuk(
								tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 9).toString(),
								tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 10).toString());
						dlgro.setVisible(true);
			...
			if (TabRawatInap.getSelectedIndex() == 0) {
				if (tbRadiologiRanap.getSelectedRow() != -1) {
					if (tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString().trim()
						...
						dlgro.emptTeks();
						dlgro.isCek();
						dlgro.setOrderFuji(tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString(),
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 1).toString(), "Ranap");
						dlgro.setDokterPerujuk(
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 9).toString(),
								tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 10).toString());
	...
	private String Status_bayar = "Belum Lunas";
		...
		try {
			...
			ps = koneksi.prepareStatement(
					"select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,permintaan_radiologi.tgl_permintaan,"
							...
							+ "permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,poliklinik.nm_poli,permintaan_radiologi.informasi_tambahan,permintaan_radiologi.diagnosa_klinis,reg_periksa.kd_pj "
							...
							+ "where permintaan_radiologi.status='ralan' and tgl_hasil='0000-00-00' and permintaan_radiologi.tgl_permintaan between ? and ? "
							+ (semua ? ""
			...
			try {
				...
				rs = ps.executeQuery();
				while (rs.next()) {
					int bayar = Sequel.cariInteger("select count(stts_bayar) from permintaan_pemeriksaan_radiologi \r\n"
							...
							+ "where no_rawat=? and stts_bayar='Belum'", rs.getString("no_rawat"));
					if (bayar <= 0) {
						Status_bayar = "Lunas";
					}
					tabMode.addRow(new String[] { rs.getString("noorder"), rs.getString("no_rawat"),
							...
							rs.getString("diagnosa_klinis"), Status_bayar,rs.getString("no_rkm_medis"),rs.getString("kd_pj") });
	...
	private void ShowRequireEx() {
		PreparedStatement prRadEx;
		PreparedStatement prDetailRadEx;
		try {
			prRadEx = koneksi.prepareStatement(
					"select no_reg,nama_pasien,tanggal_reg,jam_reg,jam_sampel,tgl_sampel,jam_hasil,tgl_hasil,nm_dokter,dokter.kd_dokter,status_bayar from reg_noRm "
							+ "Inner join dokter on dokter.kd_dokter=reg_noRm.kd_dokter "
							+ "where type_reg=1 and tanggal_reg between ? and ? and tgl_hasil=0000-00-00");

			prRadEx.setString(1, Valid.SetTgl(Tgl1.getSelectedItem() + ""));
			prRadEx.setString(2, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
			rs = prRadEx.executeQuery();
			while (rs.next()) {
				tabMode.addRow(new String[] { rs.getString("no_reg"), "", rs.getString("nama_pasien"),
						rs.getString("tanggal_reg"), rs.getString("jam_reg"), rs.getString("tgl_sampel"),
						rs.getString("jam_sampel"), rs.getString("tgl_hasil"), rs.getString("jam_hasil"),
						rs.getString("kd_dokter"), rs.getString("nm_dokter"), "Radiologi Luar", "", "",
		...
		try {
			...
			ps = koneksi.prepareStatement(
					"select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,"
							...
							+ "permintaan_radiologi.status='ralan' and tgl_hasil!='0000-00-00' and permintaan_radiologi.tgl_permintaan between ? and ? "
							+ (semua ? ""
									: "and dokter.nm_dokter like ? and poliklinik.nm_poli like ? and "
	...
	private void ShowRequireExFinal() {
		PreparedStatement prRadEx;
		PreparedStatement prDetailRadEx;
		try {
			prRadEx = koneksi.prepareStatement(
			...
			rs = prRadEx.executeQuery();
			while (rs.next()) {
				tabMode2.addRow(new String[] { rs.getString("no_reg"), "", rs.getString("nama_pasien"),
						rs.getString("tanggal_reg"), rs.getString("jam_reg"), rs.getString("tgl_sampel"),
						rs.getString("jam_sampel"), rs.getString("tgl_hasil"), rs.getString("jam_hasil"),
						rs.getString("kd_dokter"),"", rs.getString("nm_dokter"), "Radiologi Luar", "", "",
						rs.getString("status_bayar") });
		...
		try {
			...
			ps = koneksi.prepareStatement(
					"select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,"
							...
							+ " group by permintaan_radiologi.noorder,jns_perawatan_radiologi.kd_jenis_prw order by permintaan_radiologi.tgl_permintaan,permintaan_radiologi.jam_permintaan desc");
			try {
				ps.setString(1, Valid.SetTgl(Tgl1.getSelectedItem() + ""));
				ps.setString(2, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
				if (!semua) {
				...
				if (detik.equals("05")) {
					...
					if (formalarm.contains("ralan")) {
						...
						for (i = 0; i < tbRadiologiRalan.getRowCount(); i++) {
							if ((!tbRadiologiRalan.getValueAt(i, 0).toString().equals(""))
									&& tbRadiologiRalan.getValueAt(i, 5).toString().equals("")) {
								permintaanbaru++;
							}
						}
					}
					...
					if (formalarm.contains("ranap")) {
						...
						for (i = 0; i < tbRadiologiRanap.getRowCount(); i++) {
							if ((!tbRadiologiRanap.getValueAt(i, 0).toString().equals(""))
									&& tbRadiologiRanap.getValueAt(i, 5).toString().equals("")) {
								permintaanbaru++;
							}
						}
					}
