        ...
        for(i=0;i<tbData.getRowCount();i++){ 
            if(tbData.getValueAt(i,0).toString().equals("true")){
                if(tbData.getValueAt(i,1).toString().equals("Kartu Pasien 1")){
                    ...
                    param.put("logo",Sequel.cariGambar("select logo from setting")); 
                    Valid.MyReportqry("rptRM1.jasper","report","::[ Identitas Pasien ]::"," SELECT pasien.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,pasien.jk,pasien.tmp_lahir,pasien.tgl_lahir,pasien.nm_ibu,pasien.alamat, "+
                           "kelurahan.nm_kel,kecamatan.nm_kec,kabupaten.nm_kab,propinsi.nm_prop,pasien.gol_darah,pasien.pekerjaan,pasien.stts_nikah,pasien.agama,"+
                           "pasien.tgl_daftar,pasien.no_tlp,pasien.umur,pasien.pnd,pasien.keluarga,pasien.namakeluarga,penjab.png_jawab,pasien.pekerjaanpj,"+
                           "concat ( pasien.alamatpj,'-',pasien.kelurahanpj,'-',pasien.kecamatanpj,'-',pasien.kabupatenpj) as alamatpj  "+
                           "FROM pasien INNER JOIN kelurahan INNER JOIN kecamatan INNER JOIN kabupaten INNER JOIN propinsi INNER JOIN penjab ON pasien.kd_pj = penjab.kd_pj "+
                           ...
                           " where pasien.no_rkm_medis='"+NoRm+"' ",param);
                    this.setCursor(Cursor.getDefaultCursor());
                    ...
                    Valid.MyReportqry("rptLabelTracker3.jasper","report","::[ Label Tracker ]::",
                           ...
                           "reg_periksa.p_jawab,reg_periksa.almt_pj,reg_periksa.hubunganpj,reg_periksa.biaya_reg,reg_periksa.stts_daftar,penjab.png_jawab,reg_periksa.kd_pj,penjab.png_jawab "+
                           "from reg_periksa inner join dokter inner join pasien inner join poliklinik inner join penjab "+
                           "on reg_periksa.kd_dokter=dokter.kd_dokter and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                           "and reg_periksa.kd_pj=penjab.kd_pj and reg_periksa.kd_poli=poliklinik.kd_poli where reg_periksa.no_rawat='"+NoRawat+"' ",param);
                    this.setCursor(Cursor.getDefaultCursor());
