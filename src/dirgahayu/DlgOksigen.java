...
public class DlgOksigen extends JDialog {
	...
	private Table tblInputGas;
	private DefaultTableModel tabModelGas;
	...
	private Table tblDaftarGas;
	private DefaultTableModel tabModelDaftarGas;
	...
	private int Row;
	private ComboBox  masterGasMedis;
	 ...
	 * Create the dialog.
	 */
	public DlgOksigen(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		setUndecorated(true);
	...
	private void initComponents() {
		...
		internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				...
				"::[ Gas Medis ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
		...
		panelGlassTop.setPreferredSize(new java.awt.Dimension(800, 170));
		...
		jLabelTanggal.setBounds(470, 120, 41, 14);
		...
		DTPTgl.setBounds(559, 120, 90, 20);
		...
		jLabelDokter.setBounds(10, 79, 62, 14);
		jLabelDokter.setText("Nama Dokter");
		panelGlassTop.add(jLabelDokter);

		tDokter = new TextBox();
		tDokter.setBounds(180, 79, 210, 24);
		...
		jLabelPetugas.setBounds(10, 120, 78, 14);
		panelGlassTop.add(jLabelPetugas);

		tPetugas = new TextBox();
		tPetugas.setBounds(99, 115, 200, 24);
		...
		btnPetugas.setBounds(304, 115, 28, 23);
		...
		tkd_dokter.setBounds(99, 79, 78, 24);
		...
		btnDokter.setBounds(390, 79, 28, 23);
		...
		// Table Input
		tblInputGas = new widget.Table();
		tabModelGas = new DefaultTableModel(null,
				new String[] {"Waktu Awal","Waktu Akhir","Pemakaian","Jenis Gas Medis","Menit","Harga Per Liter"}) {
			@Override
		...
		tblInputGas.setModel(tabModelGas);
		tblInputGas.setPreferredScrollableViewportSize(new Dimension(500, 300));
		tblInputGas.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		for (int i = 0; i < tabModelGas.getColumnCount(); i++) {
			TableColumn column = tblInputGas.getColumnModel().getColumn(i);
			if (i == 1) {
//				column.setMinWidth(0);
//				column.setMaxWidth(0);
				DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd MMM yyyy");
				column.setCellEditor(new DateTimeTableEditor());
			} else if (i == 0) {
//				column.setMaxWidth(30);
				column.setCellEditor(new DateTimeTableEditor());
				 ...
				 masterGasMedis = new widget.ComboBox();
				 Sequel.addItemCombo("select nama_brng from databarang where kode_golongan='G56'", masterGasMedis);
				 column.setCellEditor(new DefaultCellEditor(masterGasMedis));
			}
			else if (i == 5) {
		...
		warna.kolom = 1;
		tblInputGas.setDefaultRenderer(Object.class, warna);
		tabModelGas.addRow(new Object[] {"","","","","",""});
		tblInputGas.setPreferredScrollableViewportSize(new Dimension(800, 300));
		tblInputGas.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);

		Scroll1.setViewportView(tblInputGas);
		TabRawat.addTab("Input Gas Medis", Scroll1);
		...
		TabRawat.addTab("Daftar Pemakaian Gas Medis", Scroll2);
		internalFrame1.add(TabRawat, java.awt.BorderLayout.CENTER);
//		panelGlassMiddle.add(TabRawat);
		tblDaftarGas = new widget.Table();
		tblDaftarGas.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				tblDaftarDarahMouseClicked(e);
			}
		});
		tabModelDaftarGas = new DefaultTableModel(null,
		...
		tblDaftarGas.setModel(tabModelDaftarGas);

		tblDaftarGas.setPreferredScrollableViewportSize(new Dimension(800, 400));
		tblDaftarGas.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
		for (int i = 0; i < tabModelDaftarGas.getColumnCount(); i++) {
			TableColumn column = tblDaftarGas.getColumnModel().getColumn(i);
			if (i == 1||i==7) {
				column.setMinWidth(0);
				column.setMaxWidth(0);
			} 
		...
		warna.kolom = 1;
		tblDaftarGas.setDefaultRenderer(Object.class, warna2);

		Scroll2.setViewportView(tblDaftarGas);
...
//				pasien.dispose();				
	...
	protected void BtnDeleteActionPerformed(ActionEvent e) {
		...
		if (tabModelDaftarGas.getRowCount() != 0) {	
			if (tblDaftarGas.getSelectedRow() != -1) {				
					if (tblDaftarGas.getValueAt(tblDaftarGas.getSelectedRow(), 0).toString().equals("true")) {
						if (tblDaftarGas.getValueAt(tblDaftarGas.getSelectedRow(), 0).toString().equals("Lunas")) {
							JOptionPane.showMessageDialog(rootPane,
...
//				Sequel.menyimpanNew("tindakan_darah","nomor_permintaan,no_rm,no_rawat,asal_labu,golongan_darah,resus,total_biaya,status_perawatan,kd_dokter,ruang,kd_petugas","'"+tNoPermintaan.getText()+"','"+tNoRM.getText()+"','"+tNoRw.getText()+"','"+tAsalLabu.getText()+"','"+cmbGolDarah.getSelectedItem().toString()+"','"+cmbGolRefuse.getSelectedItem().toString()+"','"+String.valueOf(total)+"','"+jnsRawat+"','"+tkd_dokter.getText()+"','"+tpoli.getText()+"','"+tPetugas.getText()+"'","Tindakan Darah");
	...
	private void emptTeks() {
		...
		for (int i = 0; i < tblInputGas.getRowCount(); i++) {
			tblInputGas.setValueAt("", i, 2);
			tblInputGas.setValueAt("0", i, 3);
		}
	}
...
//			rs = ps.executeQuery();
//			while (rs.next()) {
//				tabModelGas.addRow(new Object[] { false, rs.getString("kd_jenis_prw"), "", "0",
...
//		tabModelGas.addRow(new Object[] {"","","","","",""});
	...
	public void tampilDataPemakaianDarah() {
		sekuel Sequel = new sekuel();
		validasi Valid = new validasi();
		Valid.tabelKosong(tabModelDaftarGas);
		PreparedStatement ps;
		Connection koneksi = koneksiDB.condb();
		ResultSet rs;
		try {
			...
			while (rs.next()) {
				...
				String Penjamin=Sequel.cariIsi(
						"select png_jawab from reg_periksa Inner join penjab on penjab.kd_pj=reg_periksa.kd_pj where no_rawat=?",
						rs.getString("no_rawat"));
				tabModelDaftarGas.addRow(new Object[] { false,rs.getString("nomor_permintaan"),namaPasien,RuangRawat,Penjamin,rs.getString("total_biaya"),rs.getString("status_bayar"),rs.getString("id") });
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private void tblDaftarDarahMouseClicked(MouseEvent e) {
		...
		if (tabModelDaftarGas.getRowCount() != 0) {			
			if (tblDaftarGas.getSelectedRow() != -1) {
				...
				String noPermintaan=tblDaftarGas.getValueAt(tblDaftarGas.getSelectedRow(), 1).toString();
				...
				tPasien.setText(tblDaftarGas.getValueAt(tblDaftarGas.getSelectedRow(), 2).toString());
				tNoRw.setText(noRawat);
				...
				for (int i = 0; i < tblInputGas.getRowCount(); i++) {
					try {
						...
						rs = ps.executeQuery();
						while (rs.next()) {
							if(tblInputGas.getValueAt(i, 1).equals(rs.getString("kd_prw"))) {
								tblInputGas.setValueAt(rs.getString("nomo_kantong"), i, 2);
								tblInputGas.setValueAt(rs.getString("jumlah"), i, 3);								
							}
						}
				...
				if(tblDaftarGas.getValueAt(tblDaftarGas.getSelectedRow(),0).toString().equals("true")) {
					tblDaftarGas.setValueAt(true,tblDaftarGas.getSelectedRow(), 0);
				}else {
