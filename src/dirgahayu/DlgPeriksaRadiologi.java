...
public final class DlgPeriksaRadiologi extends javax.swing.JDialog {
	...
	public DlgPeriksaRadiologi(java.awt.Frame parent, boolean modal) {
		...
		for (i = 0; i < 19; i++) {
			...
			if (i == 0) {
				...
				if (Sequel.cariInteger(
						"select Count(nm_dokter) from dokter where kd_dokter='" + akses.getkode() + "'") == 1) {
					column.setMinWidth(0);
					column.setMaxWidth(0);
				}
				// System.out.print(akses.getkode());
			} else if (i == 11) {
		...
		tbBHP.setModel(tabMode2);

		tbBHP.setPreferredScrollableViewportSize(new Dimension(500, 500));
		tbBHP.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);

		for (i = 0; i < 5; i++) {
			TableColumn column = tbBHP.getColumnModel().getColumn(i);
			if (i == 0) {
		...
		tbBHP.setDefaultRenderer(Object.class, new WarnaTable());
	...
	private void initComponents() {
		...
		jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createLineBorder(new java.awt.Color(239, 244, 234)), " BHP Radiologi ",
				javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION,
				new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
		...
		tbBHP.setName("tbObat"); // NOI18N
		tbBHP.addMouseListener(new java.awt.event.MouseAdapter() {
			@Override
			public void mouseClicked(java.awt.event.MouseEvent evt) {
				tbObatMouseClicked(evt);
			}
		});
		tbBHP.addKeyListener(new java.awt.event.KeyAdapter() {
			@Override
			public void keyPressed(java.awt.event.KeyEvent evt) {
				tbObatKeyPressed(evt);
			}
		});
		Scroll.setViewportView(tbBHP);

		jPanel2.add(Scroll, java.awt.BorderLayout.CENTER);
		if (Sequel.cariInteger("select Count(nm_dokter) from dokter where kd_dokter='" + akses.getkode() + "'") == 1) {
			jPanel2.setVisible(false);
			jPanel3.setPreferredSize(new java.awt.Dimension(350, 102));
		} else {
			jPanel1.add(jPanel2);
		...
		Scroll3.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createLineBorder(new java.awt.Color(239, 244, 234)), " Hasil Pemeriksaan ",
				javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION,
				new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
...
//     System.out.println("No Rawat "+TNoRw.getText()+"-"+Sequel.cariIsi("SELECT nm_perawatan FROM sik.rawat_jl_drpr \n"
//        	 		+ "inner join jns_perawatan on jns_perawatan.kd_jenis_prw= rawat_jl_drpr.kd_jenis_prw where no_rawat='"+TNoRw.getText()+"'"));
//        	 
			this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));

// 			String keterangan = Sequel.cariIsi("sselect rawat_jl_drpr.no_rawat,reg_periksa.no_rkm_medis,reg_periksa.kd_poli,pasien.nm_pasien,pasien.nm_pasien,pasien.tgl_lahir,pasien.jk,pasien.umur,pasien.alamat \n"
//					+ "from rawat_jl_drpr \n"
// 					+ "inner join reg_periksa on reg_periksa.no_rawat=rawat_jl_drpr.no_rawat \n"
//					+ "inner join pasien on pasien.no_rkm_medis=reg_periksa.no_rkm_medis \n"
//					+ "where no_rawat=?",TNoRw.getText());

			String dates = Tanggal.getSelectedItem() + " " + CmbJam.getSelectedItem() + ":"
					+ CmbMenit.getSelectedItem();
			...
			param.put("nm_dockter", NmDokterPj.getText());
			param.put("nm_dockterpr", NmPerujuk.getText());
			param.put("nm_petugas", NmPtg.getText());
			param.put("no_rawat", TNoRw.getText());

//             param.put("nm_perawatanDrPd",Sequel.cariIsi("SELECT nm_perawatan FROM sik.rawat_jl_drpr \n"
//             		+ "inner join jns_perawatan on jns_perawatan.kd_jenis_prw= rawat_jl_drpr.kd_jenis_prw\n"
//             		+ "where no_rawat=?",TNoRw.getText()));
//             param.put("kasir", Sequel.cariIsi("select nama from pegawai where nik=?",akses.getkode()));
	...
	private void tbObatKeyPressed(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_tbObatKeyPressed
		if (tabMode.getRowCount() != 0) {
			if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
				try {
					// getData();
					int row = tbBHP.getSelectedColumn();
					if (row == 1) {
						TCariBhp.setText("");
	...
	private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpanActionPerformed
		if (TNoRw.getText().equals("") || TNoRM.getText().equals("") && status.equals("Rawat Jalan")
				|| TPasien.getText().equals("") && status.equals("Rawat Jalan")) {
		...
		try {
			...
			try {
				if (cara_bayar_radiologi.equals("Yes") && kelas_radiologi.equals("No")) {
					...
					if (status.equals("Radiologi Luar")) {
						pspemeriksaan4.setString(1, "Rawat Jalan");
					} else {
					...
					System.out.print(pspemeriksaan4);
	...
	public void isReset() {
		...
		jml = tbBHP.getRowCount();
		for (i = 0; i < jml; i++) {
			tbBHP.setValueAt(0, i, 0);
		}
	...
	private void tampil2() {
		jml = 0;
		for (i = 0; i < tbBHP.getRowCount(); i++) {
			// System.out.println(tbObat.getValueAt(i,0).toString());
			if (!tbBHP.getValueAt(i, 0).toString().equals("")) {
				jml++;
			}
		}
...
//			System.out.print(status);
			if (status.equals("Radiologi Luar")) {
				pspemeriksaan = koneksi.prepareStatement(
						"select jns_perawatan_radiologi.kd_jenis_prw,jns_perawatan_radiologi.nm_perawatan,jns_perawatan_radiologi.total_byr,"
								+ "jns_perawatan_radiologi.bagian_rs,jns_perawatan_radiologi.bhp,jns_perawatan_radiologi.tarif_perujuk,"
								+ "jns_perawatan_radiologi.tarif_tindakan_dokter,jns_perawatan_radiologi.tarif_tindakan_petugas,"
								+ "jns_perawatan_radiologi.kso,jns_perawatan_radiologi.menejemen,'UMUM' "
								+ "from jns_perawatan_radiologi inner join detail_reg_radiologi"
								+ " on jns_perawatan_radiologi.kd_jenis_prw=detail_reg_radiologi.kd_perawatan "
								+ "where detail_reg_radiologi.no_reg=? order by jns_perawatan_radiologi.kd_jenis_prw");
			} else {
				pspemeriksaan = koneksi.prepareStatement(
						"select jns_perawatan_radiologi.kd_jenis_prw,jns_perawatan_radiologi.nm_perawatan,jns_perawatan_radiologi.total_byr,"
								+ "jns_perawatan_radiologi.bagian_rs,jns_perawatan_radiologi.bhp,jns_perawatan_radiologi.tarif_perujuk,"
	...
	public void setOrder(String order, String norawat, String posisi) {
		...
		this.status = posisi;
		if (posisi.equals("Radiologi Luar")) {
			TNoRw.setText(order);
			TPasien.setText(Sequel.cariIsi("Select nama_pasien from reg_noRm where no_reg='" + order + "'"));
		} else {
	...
	private void simpan() {
		int reply = JOptionPane.showConfirmDialog(rootPane, "Apaka sudah Benar data yang mau disimpan..??",
		...
		if (reply == JOptionPane.YES_OPTION) {
			...
			for (i = 0; i < tbPemeriksaan.getRowCount(); i++) {
				if (tbPemeriksaan.getValueAt(i, 0).toString().equals("true")) {
					...
					if (Sequel.menyimpantf2("periksa_radiologi", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",
						...
						if (!noorder.equals("")) {
							if (Sequel.cariIsi(
									"select stts_bayar from permintaan_pemeriksaan_radiologi where noorder='" + noorder
											+ "' and kd_jenis_prw='" + tbPemeriksaan.getValueAt(i, 1).toString() + "'")
									.equals("Belum")) {
								...
								ttljmpetugas = ttljmpetugas
										+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 8).toString());
								ttlkso = ttlkso + Double.parseDouble(tbPemeriksaan.getValueAt(i, 9).toString());
								ttlpendapatan = ttlpendapatan
										+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 3).toString());
								ttljasasarana = ttljasasarana
										+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 4).toString());
								ttljmperujuk = ttljmperujuk
										+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 6).toString());
			...
			if (Sequel.cariInteger(
				...
				if (sukses == true) {
					...
					for (i = 0; i < tbBHP.getRowCount(); i++) {
						if (Valid.SetAngka(tbBHP.getValueAt(i, 0).toString()) > 0) {
							if (Sequel.menyimpantf2("beri_bhp_radiologi", "?,?,?,?,?,?,?,?", "BHP", 8, new String[] {
									TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
									CmbJam.getSelectedItem() + ":" + CmbMenit.getSelectedItem() + ":"
											+ CmbDetik.getSelectedItem(),
									tbBHP.getValueAt(i, 1).toString(), tbBHP.getValueAt(i, 3).toString(),
									tbBHP.getValueAt(i, 0).toString(), tbBHP.getValueAt(i, 4).toString(),
									Double.toString(Double.parseDouble(tbBHP.getValueAt(i, 4).toString())
											* Double.parseDouble(tbBHP.getValueAt(i, 0).toString())) }) == true) {
								ttlbhp = ttlbhp + (Double.parseDouble(tbBHP.getValueAt(i, 4).toString())
										* Double.parseDouble(tbBHP.getValueAt(i, 0).toString()));
							} else {
								sukses = false;
							}
						}
			...
			if (sukses == true) {
				if (status.equals("Ranap")) {
					...
					if (!noorder.equals("")) {
						Sequel.mengedit("permintaan_radiologi", "noorder=?", "tgl_hasil=?,jam_hasil=?", 3,
								new String[] {
					...
					sukses = jur.simpanJurnal(TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""), "U",
					...
					sukses = jur.simpanJurnal(TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""), "U",
							"PEMERIKSAAN RADIOLOGI RAWAT JALAN PASIEN " + TPasien.getText() + " DIPOSTING OLEH "
									+ akses.getkode());
				} else {
					Sequel.mengedit("reg_noRm", "no_reg=?", "status_pelayanan=?,tgl_hasil=?,jam_hasil=?", 4,
							new String[] {
									"Sudah", Valid.SetTgl(Tanggal.getSelectedItem() + ""), CmbJam.getSelectedItem()
											+ ":" + CmbMenit.getSelectedItem() + ":" + CmbDetik.getSelectedItem(),
									noorder });
