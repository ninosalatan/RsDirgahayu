...
import keuangan.DlgKasirRanapNew;
...
public class DlgKamarInap extends javax.swing.JDialog {
	...
	private String now = dateFormat.format(date), kmr = "", key = "", tglmasuk, jammasuk, kd_pj, hariawal = "",
			pilihancetak = "", aktifkan_hapus_data_salah = "";
	...
	private Button btnDiagnosaAwal;
	private ComboBox cmbKelasKamar;
	private Label jLabelKelas;
	...
	public DlgKamarInap(java.awt.Frame parent, boolean modal) {
		...
		tabMode = new DefaultTableModel(null,
				new Object[] { "No.Rawat", "Nomer RM", "Nama Pasien", "Alamat Pasien", "Penanggung Jawab",
						...
						"Dokter P.J.", "Kamar", "Status Bayar", "Agama" }) {
			@Override
		...
		diagnosaawal.setDocument(new batasInput(100).getKata(diagnosaawal));
		diagnosaakhir.setDocument(new batasInput(100).getKata(diagnosaakhir));
		DiagnosaAwalSementara.setDocument(new batasInput(100).getKata(DiagnosaAwalSementara));
		DiagnosaAkhirSementara.setDocument(new batasInput(100).getKata(DiagnosaAkhirSementara));
		...
		WindowInputKamar.setSize(680, 275);
			...
			public void windowClosed(WindowEvent e) {
				if (akses.getform().equals("DlgKamarInap")) {
					if (pasien.penjab.getTable().getSelectedRow() != -1) {
						kdpenjab.setText(pasien.penjab.getTable()
								.getValueAt(pasien.penjab.getTable().getSelectedRow(), 1).toString());
						nmpenjab.setText(pasien.penjab.getTable()
	...
	private void initComponents() {
		...
		diagnosaawal.setBounds(438, 85, 180, 23);
		...
		btnDiagnosaAwal.setName("btnDiagnosa"); // NOI18N
		...
		internalFrame2.add(btnDiagnosaAwal);
		btnDiagnosaAwal.setBounds(630, 85, 28, 23);

		diagnosaakhir.setHighlighter(null);
		diagnosaakhir.setName("diagnosaakhir");
		...
		cmbStatus.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Sehat", "Rujuk", "APS", "+",
				"Meninggal", "Sembuh", "Membaik", "Pulang Paksa", "-", "Pindah Kamar", "Status Belum Lengkap",
				"Atas Persetujuan Dokter", "Atas Permintaan Sendiri", "Lain-lain" }));
		...
		jLabelKelas = new widget.Label();
		jLabelKelas.setText("Kelas Dijamin :");
		jLabelKelas.setName("jLabelKelas");
		internalFrame2.add(jLabelKelas);
		jLabelKelas.setBounds(320, 115, 72, 23);

		cmbKelasKamar = new widget.ComboBox();
		cmbKelasKamar.setName("cmbKelasKamar");
		Sequel.addItemCombo("Select nm_kelas from kelas_kamar order by idkelas_kamar DESC", cmbKelasKamar);
		cmbKelasKamar.setBounds(400, 115, 240, 23);
		internalFrame2.add(cmbKelasKamar);
		...
		internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)),
				"::[ Daftar Pasien Rawat Inap ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
				javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11),
				new java.awt.Color(50, 50, 50))); // NOI18N
	...
	protected void btnDiagnosaAwalKeyPressed(KeyEvent evt) {
		// TODO Auto-generated method stub
		btnDiagnosaAwalActionPerformed(null);
	}

	protected void btnDiagnosaAwalActionPerformed(ActionEvent evt) {
		// TODO Auto-generated method stub
		...
		penyakit.addWindowListener(new WindowListener() {
			...
			public void windowClosed(WindowEvent e) {
				// TODO Auto-generated method stub
				if (penyakit.getTabel().getSelectedRow() != -1) {
					diagnosaawal.setText(
							penyakit.getTabel().getValueAt(penyakit.getTabel().getSelectedRow(), 2).toString());
				}
			}
	...
	private void BtnInActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnInActionPerformed
		...
		jLabelKelas.setVisible(true);
		cmbKelasKamar.setVisible(true);
		emptTeks();
	...
	private void BtnOutActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnOutActionPerformed
		if (tabMode.getRowCount() == 0) {
			...
			jLabelKelas.setVisible(false);
			cmbKelasKamar.setVisible(false);
	...
	private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnPrintActionPerformed
		...
		if (tabMode.getRowCount() == 0) {
			...
			if (!TCari.getText().equals("")) {
				key = kmr + "and (kamar_inap.no_rawat like '%" + TCari.getText().trim()
						+ "%' or reg_periksa.no_rkm_medis like '%" + TCari.getText().trim()
						+ "%' or pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or "
						+ " concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) like '%"
						+ TCari.getText().trim() + "%' or kamar_inap.kd_kamar like '%" + TCari.getText().trim()
						+ "%' or " + "bangsal.nm_bangsal like '%" + TCari.getText().trim()
						+ "%' or kamar_inap.diagnosa_awal like '%" + TCari.getText().trim()
						+ "%' or kamar_inap.diagnosa_akhir like '%" + TCari.getText().trim() + "%' or "
						+ "kamar_inap.trf_kamar like '%" + TCari.getText().trim() + "%' or kamar_inap.tgl_masuk like '%"
						+ TCari.getText().trim() + "%' or dokter.nm_dokter like '%" + TCari.getText().trim() + "%' or "
						+ "kamar_inap.stts_pulang like '%" + TCari.getText().trim()
						+ "%' or kamar_inap.tgl_keluar like '%" + TCari.getText().trim()
						+ "%' or penjab.png_jawab like '%" + TCari.getText().trim() + "%' or "
						+ "kamar_inap.ttl_biaya like '%" + TCari.getText().trim() + "%' or pasien.agama like '%"
						+ TCari.getText().trim() + "%') ";
			}

			try {
				pilihancetak = (String) JOptionPane.showInputDialog(null, "Silahkan pilih laporan yang mau dicetak!",
						"Laporan", JOptionPane.QUESTION_MESSAGE, null,
						new Object[] { "Laporan 1", "Laporan 2", "Lembar Bimbingan Rohani" }, "Laporan 1");
				switch (pilihancetak) {
				case "Laporan 1":
...
//                               "\"tglRujukan\":\""+TanggalRujuk+"")+"\"," +
//                               "\"noRujukan\":\""+NoRujukan+"\"," +
//                               "\"ppkRujukan\":\""+KdPpkRujukan+"\"" +
//                           "},"+
//                           "\"catatan\":\""+Catatan+"\"," +
//                           "\"diagAwal\":\""+diagnosaawal.getText()+"\"," +
//                           "\"poli\": {" +
			...
			if (norawat.isEditable() == true) {
				if (diagnosaawal.getText().equals("")) {
					...
					case "ISI":
						...
						break;
					case "KOSONG":
						if (Sequel.menyimpanNew("kamar_inap",
								"no_rawat,kd_kamar,trf_kamar,diagnosa_awal,tgl_masuk,jam_masuk,lama,kls_djamin",
								"'" + norawat.getText() + "','" + kdkamar.getText() + "','" + TTarif.getText() + "','"
										+ diagnosaawal.getText() + "','" + CmbTahun.getSelectedItem() + "-"
										+ CmbBln.getSelectedItem() + "-" + CmbTgl.getSelectedItem() + "','"
										+ cmbJam.getSelectedItem() + ":" + cmbMnt.getSelectedItem() + ":"
										+ cmbDtk.getSelectedItem() + "','" + TJmlHari.getText() + "','"
										+ cmbKelasKamar.getSelectedItem().toString() + "'",
								"No Rawat") == true) {
				...
				} else if (Sequel.cariIsi("select status_bayar from kamar_inap where no_rawat=?", norawat.getText())
						.equals("Belum Lunas")) {
					JOptionPane.showMessageDialog(null,
							"Pasien Belum melakukan Verifikasi Pembayaran...Silahkan Menghubungi Kasir");
				} else {
					Sequel.mengedit("kamar_inap",
							"no_rawat='" + norawat.getText() + "' and kd_kamar='" + kdkamar.getText()
									+ "' and tgl_masuk='" + TIn.getText() + "' and jam_masuk='" + JamMasuk.getText()
		...
		if (tabMode.getRowCount() != 0) {
			...
			if (evt.getClickCount() == 2) {
				...
				if (i == 0) {
					...
					if (akses.getbilling_ranap() == true) {
						DlgKasirRanapNew kasir = new DlgKasirRanapNew(null, false);
						kasir.SetNoRm(norawat.getText());
						kasir.setSize(internalFrame1.getWidth() - 20, internalFrame1.getHeight() - 20);
						kasir.setLocationRelativeTo(internalFrame1);
		...
		if (TPasienpindah.getText().trim().equals("")) {
			...
			case "KOSONG":
				if (Rganti1.isSelected() == true) {
					...
					Sequel.menyimpanFiled("kamar_inap",
							"no_rawat,kd_kamar,trf_kamar,diagnosa_awal,tgl_masuk,jam_masuk,lama,kls_djamin",
							"'" + norawatpindah.getText() + "'," + "'" + kdkamarpindah.getText() + "','" + TTarifpindah.getText() + "',"
									+ "'" + diagnosaawal.getText() + "'," + "'" + CmbTahunpindah.getSelectedItem() + "-"
									+ CmbBlnpindah.getSelectedItem() + "-" + CmbTglpindah.getSelectedItem() + "'," + "'"
									+ cmbJampindah.getSelectedItem() + ":" + cmbMntpindah.getSelectedItem() + ":"
									+ cmbDtkpindah.getSelectedItem() + "'," + "'" + TJmlHaripindah.getText() + "'," + "'"
									+ cmbKelasKamar.getSelectedItem().toString() + "'",
							"No Rawat");
					...
					if (hariawal.equals("Yes")) {
						Sequel.cariIsi("select (if(to_days('" + CmbTahunpindah.getSelectedItem() + "-"
								...
								+ tbKamIn.getValueAt(tbKamIn.getSelectedRow(), 12).toString() + "')=0,if(time_to_sec('"
								+ CmbTahunpindah.getSelectedItem() + "-" + CmbBlnpindah.getSelectedItem() + "-"
								+ CmbTglpindah.getSelectedItem() + " " + cmbJampindah.getSelectedItem() + ":"
								+ cmbMntpindah.getSelectedItem() + ":" + cmbDtkpindah.getSelectedItem()
								+ "')-time_to_sec('" + tbKamIn.getValueAt(tbKamIn.getSelectedRow(), 11).toString() + " "
					...
					isjml();
                        ...
                        Sequel.menyimpanFiled("kamar_inap",
    							"no_rawat,kd_kamar,trf_kamar,diagnosa_awal,tgl_masuk,jam_masuk,lama,kls_djamin,ttl_biaya",
    							"'" + norawatpindah.getText() + "'," + "'" + kdkamarpindah.getText() + "','" + TTarifpindah.getText() + "',"
    									+ "'" + diagnosaawal.getText() + "'," + "'" + CmbTahunpindah.getSelectedItem() + "-"
    									+ CmbBlnpindah.getSelectedItem() + "-" + CmbTglpindah.getSelectedItem() + "'," + "'"
    									+ cmbJampindah.getSelectedItem() + ":" + cmbMntpindah.getSelectedItem() + ":"
    									+ cmbDtkpindah.getSelectedItem() + "'," + "'" + TJmlHaripindah.getText() + "'," + "'"
    									+ cmbKelasKamar.getSelectedItem().toString() + "','"+ttlbiayapindah.getText()+"'",
    							"No Rawat");
                        Sequel.mengedit("kamar","kd_kamar='"+kdkamarpindah.getText()+"'","status='ISI'");                         
					...
					Sequel.mengedit("kamar_inap",
							"no_rawat='" + tbKamIn.getValueAt(tbKamIn.getSelectedRow(), 0).toString()
									...
									+ "' and jam_masuk='" + tbKamIn.getValueAt(tbKamIn.getSelectedRow(), 12).toString()
									+ "'",
							"trf_kamar='" + TTarifpindah.getText() + "',tgl_keluar='" + CmbTahunpindah.getSelectedItem()
									+ "-" + CmbBlnpindah.getSelectedItem() + "-" + CmbTglpindah.getSelectedItem()
									+ "',jam_keluar='" + cmbJampindah.getSelectedItem() + ":"
									+ cmbMntpindah.getSelectedItem() + ":" + cmbDtkpindah.getSelectedItem()
									+ "',ttl_biaya='" + ttlbiaya.getText() + "',lama='" + TJmlHari.getText()
									+ "',stts_pulang='Pindah Kamar'");
					Sequel.mengedit("kamar",
							"kd_kamar='" + tbKamIn.getValueAt(tbKamIn.getSelectedRow(), 19).toString() + "'",
							"status='KOSONG'");
...
//                diagnosa.setVisible(true);
	...
	private void BtnHapusGabungActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnHapusGabungActionPerformed
		Sequel.mengedit("reg_periksa", "no_rawat='"
				+ Sequel.cariIsi("select no_rawat2 from ranap_gabung where no_rawat=?", norawat.getText()) + "'",
				"status_lanjut='Ralan'");
		...
		if (tabMode.getRowCount() == 0) {
			...
			if (tbKamIn.getSelectedRow() > -1) {
				if (Sequel.cariRegistrasi(norawat.getText()) > 0) {
					...
					NoRawatGabung.setText("");
					Sequel.cariIsi("select no_rawat2 from ranap_gabung where no_rawat=?", NoRawatGabung,
							norawat.getText());
	...
	private void MnPeniliaianAwalKeperawatanKebidananActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_MnPeniliaianAwalKeperawatanKebidananActionPerformed
		if (tabMode.getRowCount() == 0) {
			...
			if (tbKamIn.getSelectedRow() > -1) {
				this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				RMPenilaianAwalKeperawatanKebidananRanap form = new RMPenilaianAwalKeperawatanKebidananRanap(null,
						false);
				form.isCek();
				form.emptTeks();
				form.setNoRm(norawat.getText(), DTPCari2.getDate());
							...
							if (rs2.next()) {
								...
								try {
									...
									if (rs.next()) {
										...
										form.setNoRm(rs.getString("no_rawat"), rs.getString("no_sep"),
												rs.getString("no_kartu"), rs.getString("nomr"),
												rs.getString("nama_pasien"), rs.getString("tanggal_lahir"),
					...
					try {
						...
						if (rs.next()) {
							if (rs.getString("no_peserta").length() < 13) {
								...
								form.setNoRm(TNoRw1.getText(), rs.getString("no_peserta"), TNoRM.getText(),
	...
	private void tampil() {
		...
		if (!TCari.getText().equals("")) {
			key = kmr + "and (kamar_inap.no_rawat like '%" + TCari.getText().trim()
					...
					+ "%' or pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or "
					+ " concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) like '%"
					+ TCari.getText().trim() + "%' or kamar_inap.kd_kamar like '%" + TCari.getText().trim() + "%' or "
					+ "bangsal.nm_bangsal like '%" + TCari.getText().trim() + "%' or kamar_inap.diagnosa_awal like '%"
					+ TCari.getText().trim() + "%' or kamar_inap.diagnosa_akhir like '%" + TCari.getText().trim()
					+ "%' or " + "kamar_inap.trf_kamar like '%" + TCari.getText().trim()
					+ "%' or kamar_inap.tgl_masuk like '%" + TCari.getText().trim() + "%' or dokter.nm_dokter like '%"
					+ TCari.getText().trim() + "%' or " + "kamar_inap.stts_pulang like '%" + TCari.getText().trim()
					+ "%' or kamar_inap.tgl_keluar like '%" + TCari.getText().trim() + "%' or penjab.png_jawab like '%"
					+ TCari.getText().trim() + "%' or " + "kamar_inap.ttl_biaya like '%" + TCari.getText().trim()
	...
	private void isKmr() {
		if (i == 1) {
			...
			try {
				hargakamar = 0;
				ps = koneksi.prepareStatement(
						"select kamar.kd_kamar,kamar.status,bangsal.kd_bangsal,bangsal.nm_bangsal,kamar.trf_kamar "
	...
	public void isCek() {
		...
		/*
		 * if((MnHemodialisa.isVisible()==false)&&(ppAsuhanGizi.isVisible()==false)&&(
		 * ppMonitoringAsuhanGizi.isVisible()==false)&&(ppResume.isVisible()==false)&&(
		 ...
		 * 
		 * if((MnRawatJalan.isVisible()==false)&&(MnRawatInap.isVisible()==false)&&(
		 * MnPeriksaLab.isVisible()==false)&&(MnPeriksaRadiologi.isVisible()==false)&&(
		 * MnOperasi.isVisible()==false)&&(MnDiet.isVisible()==false)){
		 * MnTindakan.setVisible(false); }
		 * 
		 * if((MnPemberianObat.isVisible()==false)&&(MnInputResep.isVisible()==false)&&(
		 * MnResepDOkter.isVisible()==false)&&(MnNoResep.isVisible()==false)&&(
		 * MnStokObatPasien.isVisible()==false)&&(MnReturJual.isVisible()==false)&&(
		 * MnResepPulang.isVisible()==false)&&(MnPenjualan1.isVisible()==false)){
		 * MnObat.setVisible(false); }
		 * 
		 * if((MnRujukMasuk.isVisible()==false)&&(MnRujuk.isVisible()==false)){
		 * MnRujukan.setVisible(false); }
		 * 
		 * if((MnCekKepesertaan.isVisible()==false)&&(MnCekNIK.isVisible()==false)&&(
		 * MnSEP.isVisible()==false)&&(MnPCare.isVisible()==false)&&(MnRujukSisrute.
		 * isVisible()==false)&&(ppPasienCorona.isVisible()==false)&&(ppPerawatanCorona.
		 * isVisible()==false)&&(MnTeridentifikasiTB.isVisible()==false)){
		 * MenuBPJS.setVisible(false); }
		 * 
		 * if((ppCatatanPasien.isVisible()==false)&&(ppDataHAIs.isVisible()==false)&&(
		 * ppBerkasDigital.isVisible()==false)&&(ppIKP.isVisible()==false)){
