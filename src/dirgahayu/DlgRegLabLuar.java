...
public class DlgRegLabLuar extends JDialog {
	...
	private DefaultTableModel tabModelTidakan, tabModelReg;
	...
	public static void main(String[] args) {
		java.awt.EventQueue.invokeLater(() -> {
			DlgRegLabLuar dialog = new DlgRegLabLuar(new javax.swing.JFrame(), true);
	 ...
	 * Create the dialog.
	 */
	public DlgRegLabLuar(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		setUndecorated(true);
//		setResizable(false);
		initComponents();
	...
	private void initComponents() {
		...
		internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				...
				"::[ Input Pendaftaran  ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
...
//		});
		...
		TabWo.addTab("Tindakan ", internalFrameTabWo);
...
//                BtnAllPeriksaKeyPressed(evt);
		...
		tabModelTidakan = new DefaultTableModel(null,
		...
		tbPemeriksaan.setModel(tabModelTidakan);
		...
		for (int i = 0; i < tabModelTidakan.getColumnCount(); i++) {
...
//		 Regiter
		...
		TabWo.addTab("Registrasi ", internalFrameTabReg);
		...
		tbReg = new widget.Table();
		tabModelReg = new DefaultTableModel(null, new Object[] { "P", "No Periksa", "Nama Pasien" }) {
			@Override
			public boolean isCellEditable(int rowIndex, int colIndex) {
				boolean a = false;
				if (colIndex == 0) {
		...
		tbReg.setModel(tabModelReg);
		tbReg.setPreferredScrollableViewportSize(new Dimension(800, 800));
		tbReg.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
		for (int i = 0; i < tabModelReg.getColumnCount(); i++) {
			TableColumn column = tbReg.getColumnModel().getColumn(i);
			if (i == 0) {
		...
		tbReg.addMouseListener(new java.awt.event.MouseAdapter() {
			@Override
			public void mouseClicked(java.awt.event.MouseEvent evt) {
				tabRegMouseClicked(evt);
			}
		});
		tbReg.addKeyListener(new java.awt.event.KeyAdapter() {
			@Override
			public void keyPressed(java.awt.event.KeyEvent evt) {
				tabRegKeyPressed(evt);
			}
		});

		ScrollReg.setViewportView(tbReg);
	...
	protected void BtnNotaJasperActionPerformed(ActionEvent e) {
		...
		if (TabWo.getSelectedIndex() == 1) {
			if (tbReg.getSelectedRow() != -1) {
				this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				String noorder = tbReg.getValueAt(tbReg.getSelectedRow(), 1).toString().trim();
				...
				Valid.MyReportqry("notaLuar.jasper", "report", "::[ Billing  & Pemeriksaan Luar]::",
						"Select ROW_NUMBER() OVER(PARTITION BY reg_noRm.no_reg) as no,jns_perawatan_lab.nm_perawatan,jns_perawatan_lab.total_byr "
								+ "nama_pasien,alamat,tgl_lahir,umur,jeni_kelamin as jk from reg_noRm "
								+ "inner join detail_reg_lab on detail_reg_lab.no_reg=reg_noRm.no_reg "
								+ "inner join jns_perawatan_lab on jns_perawatan_lab.kd_jenis_prw=detail_reg_lab.kd_perawatan "
	...
	protected void btnHapusReg() {
		// TODO Auto-generated method stub
		for (i = 0; i < tbReg.getRowCount(); i++) {
			if (tbReg.getValueAt(i, 0).toString().equals("true")) {
				if (Sequel.cariInteger("Select count(no_reg) as reg from  detail_reg_lab where no_reg='"
						+ tbReg.getValueAt(tbReg.getSelectedRow(), 1)
					...
					if (jawab == JOptionPane.YES_OPTION) {
						Sequel.meghapus("detail_reg_lab", "no_reg",
								tbReg.getValueAt(tbReg.getSelectedRow(), 1).toString());
	...
	protected void tabRegKeyPressed(KeyEvent evt) {
		// TODO Auto-generated method stub
		if (tabModelReg.getRowCount() != 0) {
			if (evt.getKeyCode() == KeyEvent.VK_SPACE) {
				i = tbReg.getSelectedColumn();
				 if (i == 1) {
	...
	protected void tabRegMouseClicked(MouseEvent evt) {
		// TODO Auto-generated method stub
		if (tabModelReg.getRowCount() != 0) {
			try {
				getDataReg();
	...
	private void getDataReg() {
		...
		if (tbReg.getSelectedRow() != -1) {
			String Noreg = tbReg.getValueAt(tbReg.getSelectedRow(), 1).toString();
	...
	public void isReset() {
		...
		Valid.tabelKosong(tabModelTidakan);
		tampil();
	}
				...
				try {
					for (i = 0; i < tbPemeriksaan.getRowCount(); i++) {
						if (tbPemeriksaan.getValueAt(i, 0).toString().equals("true")) {
							ps1 = koneksi.prepareStatement(
									"Insert into detail_reg_lab (no_reg,kd_perawatan,harga,total_bayar,status_bayar) VALUES ('"
					...
					if (Status == true) {
						ps = koneksi.prepareStatement("insert into reg_noRm (no_reg,\r\n" + "tanggal_reg,\r\n"
								...
								+ "','" + cmbWn.getSelectedItem().toString() + "','2','" + kd_dokter
...
//					System.out.print(ps);
				...
				if (Check == 1) {
					...
					if (reply == JOptionPane.YES_OPTION) {
						try {
							Sequel.meghapus("detail_reg_lab", "no_reg", txtNoReg.getText());
							for (i = 0; i < tbPemeriksaan.getRowCount(); i++) {
								if (tbPemeriksaan.getValueAt(i, 0).toString().equals("true")) {
	...
	private void autoNomor() {
		...
		Valid.autoNomer3(
				...
				"LAB/" + dateFormat.format(now).replaceAll("-", "/") + "/", 6, txtNoReg);
	}

	private void TampilRegistrasi() {
		Valid.tabelKosong(tabModelReg);
	...
	private void tampil() {
		...
		try {
			...
			Valid.tabelKosong(tabModelTidakan);
			for (i = 0; i < jml; i++) {
				tabModelTidakan.addRow(new Object[] { pilih[i], kode[i], nama[i] });
			}
			String query = "";
			...
			if (!TCariPeriksa.getText().trim().equals("")) {
				condition = " and jns_perawatan_lab.nm_perawatan like '%" + TCariPeriksa.getText().trim() + "%' ";
			}
			if (isEdit == true) {
				query = "select jns_perawatan_lab.kd_jenis_prw,jns_perawatan_lab.nm_perawatan,jns_perawatan_lab.total_byr,"
						...
						+ "jns_perawatan_lab.kso,jns_perawatan_lab.menejemen,penjab.png_jawab, IF(detail_reg_lab.no_reg IS NULL,0,1) as reg_tindakan "
						+ "from jns_perawatan_lab "
						+ "inner join penjab on penjab.kd_pj=jns_perawatan_lab.kd_pj "
						+ "left join detail_reg_lab on detail_reg_lab.kd_perawatan=jns_perawatan_lab.kd_jenis_prw and detail_reg_lab.no_reg='"
						+ txtNoReg.getText() + "' "
						+ "where jns_perawatan_lab.status='1' and jns_perawatan_lab.kelas='Rawat Jalan' and penjab.png_jawab='UMUM' "
						+ condition + " order by jns_perawatan_lab.kd_jenis_prw";
			} else {
