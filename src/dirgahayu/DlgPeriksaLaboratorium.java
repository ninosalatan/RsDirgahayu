...
public final class DlgPeriksaLaboratorium extends javax.swing.JDialog {
	...
	private PreparedStatement pstindakan, pstindakan2, pstindakan3, pstindakan4, pstampil, pstampil2, pstampil3,
			pstampil4, pslica, pscariperawatan, psset_tarif, pssetpj, psrekening;
...
// 	 this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
// 	 String dates=Tanggal.getSelectedItem()+" "+CmbJam.getSelectedItem()+":"+CmbMenit.getSelectedItem();
//      Map<String, Object> param = new HashMap<>();
//      param.put("namars",akses.getnamars());
//      param.put("alamatrs",akses.getalamatrs());
//      param.put("kotars",akses.getkabupatenrs());
...
//              "select ROW_NUMBER() OVER(PARTITION BY a.noorder) as no,c.nm_perawatan,c.total_byr from permintaan_lab a  "
//              + "INNER JOIN permintaan_pemeriksaan_lab b inner join jns_perawatan_lab c on on a.noorder = b.noorder and b.kd_jenis_prw = c.kd_jenis_prw "
//              + " where a.noorder ='"+noorder+"' ",param);
//          this.setCursor(Cursor.getDefaultCursor());
...
//      param.put("kasir", Sequel.cariIsi("select nama from pegawai where nik=?",akses.getkode()));
			...
			Valid.MyReportqry("notaLab.jasper", "report", "::[ Billing Laboratorium & Pemeriksaan ]::",
					...
					" SELECT " + "ROW_NUMBER() OVER ( PARTITION BY permintaan_pemeriksaan_lab.noorder ) AS NO,"
							...
							+ " permintaan_pemeriksaan_lab.noorder='" + noorder + "' ",
					param);
//    		  
//              "select ROW_NUMBER() OVER(PARTITION BY noorder) as no,nm_perawatan,total_byr from permintaan_pemeriksaan_lab "
//              + "INNER JOIN jns_perawatan_lab on permintaan_pemeriksaan_lab.kd_jenis_prw=jns_perawatan_lab.kd_jenis_prw  "
//              + " where noorder='"+noorder+"' ",param);
			this.setCursor(Cursor.getDefaultCursor());
		}
	...
	private void simpanlab() {
		...
		try {
				...
				if (tbTarif.getValueAt(i, 0).toString().equals("true")) {
					if (Sequel.menyimpantf2("periksa_lab", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'PK',?,?",
							...
							new String[] { TNoRw.getText(), KdPtg.getText(), tbTarif.getValueAt(i, 1).toString(),
									...
									tbTarif.getValueAt(i, 3).toString(), KodePj.getText(), status, "0",
									"0" }) == true) {
						if (!noorder.equals("")) {
							if (Sequel.cariIsi("select stts_bayar from permintaan_pemeriksaan_lab where noorder='"
									+ noorder + "' and kd_jenis_prw='" + tbTarif.getValueAt(i, 1).toString() + "'")
									.equals("Belum")) {
						...
						try {
							...
							if (rscari.next()) {
								if (Sequel.menyimpantf2("detail_periksa_lab", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",
										"ID Template", 16,
										new String[] { TNoRw.getText(), rscari.getString(1),
												Valid.SetTgl(Tanggal.getSelectedItem() + ""),
												CmbJam.getSelectedItem() + ":" + CmbMenit.getSelectedItem() + ":"
														+ CmbDetik.getSelectedItem(),
									...
									if (!noorder.equals("")) {
										if (Sequel.cariIsi(
												"select stts_bayar from permintaan_detail_permintaan_lab where noorder='"
														+ noorder + "' and kd_jenis_prw='" + rscari.getString(1)
														+ "' and id_template='"
														+ tbPemeriksaan.getValueAt(i, 6).toString() + "'")
												.equals("Belum")) {
											ttlbhp = ttlbhp
													+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 9).toString());
											...
											ttlkso = ttlkso
													+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 13).toString());
											...
											ttlmenejemen = ttlmenejemen
													+ Double.parseDouble(tbPemeriksaan.getValueAt(i, 14).toString());
										}
									} else {
			...
			if (sukses == true) {
				if (!noorder.equals("")) {
					Sequel.mengedit("permintaan_lab", "noorder=?", "tgl_hasil=?,jam_hasil=?", 3,
							new String[] { Valid.SetTgl(Tanggal.getSelectedItem() + ""), CmbJam.getSelectedItem() + ":"
									+ CmbMenit.getSelectedItem() + ":" + CmbDetik.getSelectedItem(), noorder });
				}
				...
				if (status.equals("Ranap")) {
					...
					sukses = jur.simpanJurnal(TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""), "U",
							"PEMERIKSAAN LABORAT RAWAT JALAN PASIEN " + TPasien.getText() + " DIPOSTING OLEH "
									+ akses.getkode());
				}
			}
